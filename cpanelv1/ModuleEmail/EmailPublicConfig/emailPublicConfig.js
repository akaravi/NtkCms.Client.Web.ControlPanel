app.controller("emailPublicConfigCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$builder', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $builder, $state, $filter) {
    ...
    emailPublicConfig.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmssite/exportfile', emailPublicConfig.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            emailPublicConfig.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    emailPublicConfig.onSelection = function (node, selected) {
        ...
        emailPublicConfig.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            emailPublicConfig.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
