...
quoteContent.LinkCategoryIdSelector = {
    ...
    quoteContent.selectContent = function (node) {
        ...
        if(node !=null && node != undefined)
        {
            ...
            quoteContent.attachedFiles = null;
            quoteContent.attachedFiles = [];
            var s = {
    ...
    quoteContent.addNewContentModel = function () {
        ...
        var node = quoteContent.treeConfig.currentNode;
        if (node.Id == 0 || !node.Id) {
            rashaErManage.showMessage($filter('translate')('To_Add_A_Quote_Please_Select_The_Category'));
            return;
        }
        quoteContent.attachedFiles = [];
        quoteContent.attachedFile = "";
 ...
 //#help similar & otherinfo
   ...
   quoteContent.editOtherInfo = function(y) {
    ...
    quoteContent.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", quoteContent.fileIdToDelete, 'GET').success(function (response1) {
    ...
    quoteContent.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", quoteContent.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    quoteContent.onSelection = function (node, selected) {
        ...
        quoteContent.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            quoteContent.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
