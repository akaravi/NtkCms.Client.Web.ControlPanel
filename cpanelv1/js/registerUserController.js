app.controller("registerUserCtrl", ["$scope", "ajax", "rashaErManage", "$state", "$element", "$rootScope", "notify", function ($scope, ajax, rashaErManage, $state, $element, $rootScope, notify) {
    ...
    register.init = function () {
        ajax.call(mainPathApi+'cmsUser/getAntiForgeryToken', {}, 'POST').success(function (res) {
    ...
    register.gotoStepTwo = function (param) {
        ...
        if (param.$valid) {
            if (register.selectedItem.Username.substring(0, 2) == "09") {
                ...
                ajax.call(mainPathApi+"cmsUser/GetActivationCode", { userName: register.selectedItem.Username }, "post").success(function (response) {
                    register.sendSmsActivationCodeBusyIndicator.isActive = false;
                    if (response.IsSuccess == true) {
                        notify({ message: 'کد فعالسازی برای شما ارسال شد', classes: 'alert-success', templateUrl: template });
                        register.sendSmsActivationCodeBusyIndicator.isActive = true;
                        ajax.call(mainPathApi+'cmsUser/getviewmodel',  0 , 'POST').success(function (res) {
                            register.sendSmsActivationCodeBusyIndicator.isActive = false;
                            if (res.IsSuccess) {
    ...
    register.gotoStepThree = function (param) {
        if (param.$valid) {
            ...
            ajax.call(mainPathApi+"cmsUser/VerifyActivationCode", { username: register.selectedItem.Username, activecode: register.selectedItem.ActiveCode }, "POST").success(function (response) {
    ...
    register.gotoStepFour = function (param) {
        if (param.$valid) {
            ...
            ajax.call(mainPathApi+"cmsUser/add", ds , "POST").success(function (response) {
    ...
    register.updateProfile = function (param) {
        if (param.$valid) {
            var ds = register.selectedItem;
            ajax.call(mainPathApi+"cmsUser/edit",  ds , "PUT").success(function (response) {
