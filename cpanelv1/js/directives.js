    ...
    return {
        ...
        link: function (scope, element, attrs) {
            scope.offset = function () {
                var rec = document.getElementById(config.mainDrag).getBoundingClientRect(), bodyElt = document.body;
                return {
            ...
            element.draggable({ containment: "#" + config.mainDrag });
            //@help@ function 
                    ...
                    if (config.columnOptions.columns[i].enableSearch == undefined || config.columnOptions.columns[i].enableSearch == true) {
                        if (isnum) {
                            if (config.columnOptions.columns[i].type == 'integer' || config.columnOptions.columns[i].name == 'Id')
                                data.Filters.push({ PropertyName: config.columnOptions.columns[i].name, SearchType: 10, IntValue1: config.filterText, ClauseType: 1 });
                        }
                        if (config.columnOptions.columns[i].type == undefined || config.columnOptions.columns[i].type == 'string')
                            data.Filters.push({ PropertyName: config.columnOptions.columns[i].name, SearchType: 5, StringValue1: config.filterText, ClauseType: 1 });
                    }
                }
...
function rashaGrid($compile, $rootScope, ajax) {
    return {
        ...
        link: function (scope, element) {
            ...
            angular.forEach(config.columns, function (item, key) {
                item.srcThumbnail = '{{infoDomainAddress}}mvc/CmsFileContent/Thumbnail/{{x.' + item.name + '}}?Authorization=' + encodeURIComponent(localStorage.getItem('userGlobaltoken'));
                if (!item.visible)
                    item.visible = true;
                if (!item.sortable)
                    item.sortable = false;
                    ...
                    if (!item.template) {
                        if (item.isEditable) {
                            ...
                            if (item.isCheckBox) {
                                ...
                                //template = '<td style="width:{{x.width}}"><div  ng-show="systemColumnOptions.resultAccess.AccessWatchField.indexOf(\'' + item.name + '\')>-1">{{x.' + item.name + '}}<a class="btn" rel="popoverThumbnail" data-img="{{x.' + item.name + '}}" data-width="' + item.widthImg + '" data-height="' + item.heightImg + '"><img src="/CmsFiles/img/default-grid-img.png" width="50" height="50"/></a></div></td>';
                                //template = '<td style="width:{{x.width}}"><div  ng-show="systemColumnOptions.resultAccess.AccessWatchField.indexOf(\'' + item.name + '\')>-1">{{x.' + item.name + '}}{{x.' + item.name + '|customPopover}}</div></td>';
                                ...
                                //template = template + '<img ng-hide="\'{{x.' + item.name + '}}\'==\'\'" ng-show="\'{{x.' + item.name + '}}\'!=\'\'" style="width:{{' + item.imageWidth + '}}px;height:{{' + item.imageHeight + '}}px" ng-src="' + item.srcThumbnail + '" >';
                                ...
                                if (item.filter != undefined) {
                                    template = '<td style="{{x.style}}"><div  ng-show="{{' + item.visible + '}}">{{x.' + item.name + ' |' + item.filter + ':"' + item.name + '"}}</div></td>';
                                }
                                else {
                                    template = '<td style="{{x.style}}"  ng-show="{{' + item.visible + '}}"><div>{{x.' + item.name + '}}</div></td>';
                                    //template = '<td style="width:{{x.width}}"  ><div>{{x.' + item.name + '}}   </div></td>';
                                }
            ...
            config.selectRow = function (opt) {
                ...
                if (config.multiSelect)
                    ...
                    opt.isSelected = true;
                    config.selectedRow = { item: opt.rowItem, rowIndex: opt.rowIndex, rowData: opt };
                ...
                if (!$.isEmptyObject(result)) {
                    for (var i = 0; i < result.rules.length; i++) {
                        ...
                        if (result.rules[i].type == "integer") {
                            if (searchType == 4) Filter = { PropertyName: propertyName, IntValue1: value[0], IntValue2: value[1], SearchType: searchType, ClauseType: clauseType };
                            else Filter = { PropertyName: propertyName, IntValue1: value, SearchType: searchType, ClauseType: clauseType };
                        }
                        else if (result.rules[i].type == "string" && result.rules[i].input != "radio") {
                            if (searchType == 4) Filter = { PropertyName: propertyName, StringValue1: value, StringValue2: value, SearchType: searchType, ClauseType: clauseType };
                            else Filter = { PropertyName: propertyName, StringValue1: value, SearchType: searchType, ClauseType: clauseType };
                        }
                        else if (result.rules[i].type == "boolean") {    // Type is boolean
                            if (searchType == 4) Filter = { PropertyName: propertyName, BooleanValue1: value[0], BooleanValue2: value[1], SearchType: searchType, ClauseType: clauseType };
                            else Filter = { PropertyName: propertyName, BooleanValue1: value, SearchType: searchType, ClauseType: clauseType };

                        }
                        else if (result.rules[i].type == "date") {
                            if (searchType == 4) Filter = { PropertyName: propertyName, DateTimeValue1: value[0], DateTimeValue2: value[1], SearchType: searchType, ClauseType: clauseType };
                            else Filter = { PropertyName: propertyName, DateTimeValue1: value, SearchType: searchType, ClauseType: clauseType };
                        }
                        Filters.push(Filter);
                    }
                    config.gridOptions.advancedSearchData.engine.Filters = null;
...
function rashaErManage($state, notify, SweetAlert) {
    ...
    this.checkAction = function (response, errCode, c, d) {
        if (response) {
            ...
            if (!response.ResponseError) {
                ...
                notify({ message: 'Show Log For More Error', classes: 'alert-danger', templateUrl: template });
                return;
            ...
            if (response.ResponseError == 200) {
                ...
                if (response.IsSuccess != undefined && response.IsSuccess == false) {
                    ...
                    notify({ message: myMessage, classes: 'alert-danger', templateUrl: template });
                ...
                if ((Date.now() - lastErroLogin) > 1000)
                    notify({ message: 'لطفاَ ابتدا وارد سایت شوید', classes: 'alert-danger', templateUrl: template });
                ...
                if (response.ErrorMessage != '')
                    notify({ message: response.ErrorMessage, classes: 'alert-warning', templateUrl: template });
    ...
    this.showMessage = function (message) {
        notify({
            message: message, classes: 'alert-warning', templateUrl: template
        });
    }
...
function ajax($http, $state) {
        ...
        return $http(
            {
                ...
                'data': data,
                'async': isasync,
                'headers': { 'Content-Type': 'application/json', 'Authorization': userglobaltoken }
    ...
    this.logOut = function () {
        ...
        $.ajax({
            type: "POST",
            data: data,
            url: mainPathApi + "CmsUser/UserClearToken",
    ...
    return {
        ...
        link: function (scope, element) {
            ...
            if (atr.indexOf(".") > 0) {
                ...
                config = scope[atr];
            if (config != undefined)
                config.pictureUrl = "CmsFiles/img/loading.png";
    ...
    return {
        ...
        link: function (scope, el, attrs) {
            ...
            scope.content = function () {
                ajax.call(mainPathApi + 'CmsFileContent/getviewmodel', attrs.popoverId, 'GET').success(function (response) {
                    $("#image" + attrs.popoverId).attr("src", "/files/" + response.Item.Id + "/" + response.Item.FileName);
                }).error(function (data, errCode, c, d) {
                    console.log(data);
                });
                return '<img id="image' + attrs.popoverId + '" src="CmsFiles/img/loader.gif"  width="' + attrs.width + '" height="' + attrs.height + '"/>';
            };
            $(el).popover({
                trigger: 'click',
                html: true,
            ...
            function getImageSize(src) {
                ...
                return { width: orgWidth, heigth: orgHeight };
            }
        }
    }
}
...
function rashaFilePickerB($compile, ajax, $http) {
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            config.init = function () {
                ajax.call(mainPathApi + "CmsFileCategory/getall", { RowPerPage: 150 }, 'POST').success(function (response) {
            ...
            config.getCategoryFiles = function (id) {
                ...
                ajax.call(mainPathApi + "CmsFileContent/GetFilesFromCategory", id, 'POST').success(function (response) {
                    config.FileList = [];
            ...
            config.itemClicked = function ($event, index, type) {
                if (type == 'file' || type == 1) {
                    ...
                    config.fileId = index;
                    config.filename = excerpt(getFileName(config.FileList, index));
            ...
            config.initViews = function (backElement) {
                ...
                var openFileUploadModal = '<div id="test1" class="modal fade" role="dialog" style="z-index: 1400;">' +
                    ...
                    '<div class="webix_view webix_control webix_el_segmented webix_fmanager_modes" view_id="$segmented1" style="float: left; display: inline-block; vertical-align: top; border-width: 0px; margin-top: 5px; margin-left: 7px; width: 100%; height: auto;" flow-init="{target: \'' + mainPathApi + 'CmsFileContent/upload\'}"' +
                ...
                //config.filename = config.pictureUrl.toString().substring(config.pictureUrl.toString().lastIndexOf('/') + 1);
                ...
                //var template = removeSelectedFile_btn + '<div ng-show="??config??.isActive"  class="rashaFilePicker"><button class="btn btn-outline btn-primary dim margin" type="button" data-toggle="modal" data-target="#chooseUploadModal"><i class="fa fa-file"></i></button></div>' + chooseUploudTypePanel;
                ...
                //var template ='<div ng-show="??config??.isActive"  class="rashaFilePicker"><label id="file{{??config??.fileId}}" value="{{??config??.fileId}}" >{{??config??.filename}}</label><button class="btn btn-outline btn-primary dim margin" onclick="ShowUploadPanel()" type="button" ' + backElement + '"><i class="fa fa-file"></i></button></div>';
                template = template.replaceAll("??config??", atr);
                var el = $compile(template)(scope);
                element.append(el);
                config.init();
            ...
            //config.removeSelectedfile = function (config) {
            //    config.fileId = null;
                ...
                if ($("#save-icon" + index).hasClass("fa-save")) {
                    if (fileIsExist(name)) { // File already exists
                        if (confirm('File "' + name + '" already exists! Do you want to replace the new file?')) {
                            ...
                            // Delete the file
                            ajax.call(mainPathApi + "CmsFileContent/getviewmodel", fileIdToDelete, 'GET').success(function (response1) {
                                        ...
                                        if (response2.IsSuccess == true) {
                                            ...
                                            ajax.call(mainPathApi + "CmsFileContent/getviewmodel", "0", 'GET').success(function (response3) {
                                                if (response3.IsSuccess == true) {
                                                    ...
                                                    // ------- fdm.saveNewFile()  ----------------------
                                                    var result = 0;
                                                    ajax.call(mainPathApi + "CmsFileContent/add", FileItem, 'POST').success(function (response) {
    ...
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            config.replaceFile = function (name) {
                ...
                // Delete the file
                ajax.call(mainPathApi + "CmsFileContent/getviewmodel", config.fileIdToDelete, 'GET').success(function (response1) {
                    if (response1.IsSuccess == true) {
                        //console.log(response1.Item);
                        ajax.call(mainPathApi + 'CmsFileContent/delete', response1.Item, 'DELETE').success(function (response2) {
            ...
            config.saveNewFile = function () {
                ajax.call(mainPathApi + "CmsFileContent/add", config.FileItem, 'POST').success(function (response) {
                    if (response.IsSuccess) {
...
function rashaAddMenu($compile, ajax, rashaErManage) {
    return {
        restrict: 'EA',
        scope: {
            title: "@title", config: "=config", setting: "@setting"
        },
        link: function (scope, element) {

            var listItems = {};
            scope.menuItem = "x";
            ajax.call(mainPathApi + "cmsSiteMenu/getall", "", 'POST').success(function (response) {
            ...
            scope.submitMenu = function () {
                if (scope.menuItem == "x") {
                    ...
                    ajax.call(mainPathApi + 'cmsSiteMenu/edit', scope.menuDrop[objectId], 'PUT').success(function (response) {
                        rashaErManage.checkAction(response);
                        if (response.IsSuccess) {
        ...
        link: function (scope, element, attrs) {
            ...
            scope[frameId].draw = function (elem) {
                ...
                for (var i = 0; i < scope[frameId].option.maxCount; i++) {
                    ...
                    elem.append(x);
                    scope[frameId].drawingItems.push({ Id: divId, firstleft: left, left: left, top: top, firsttop: top, firstwidth: width, firstheight: height });
                }
            }
        ...
        link: function (scope, element) {
            ...
            config.uploader_filesSelected = function (files) {
                for (var i = 0; i < files.length; i++) {
                    config.files_selectedToUpload.push({ file: files[i], name: files[i].name, hasChecked: true, progress_total: files[i].size, progress_value: 0 })
                }
                scope.$apply();
            }
    ...
    return {
        ...
        link: function (scope, element) {
            ...
            if (config.defaultFilter == undefined)
                config.defaultFilter = { RowPerPage: 500, PageNumber: 0 };
            ...
            config.deleteFileFolder = function () {
                ...
                var urlViewModel = mainPathApi + 'CmsFileContent/getviewmodel';
                var urlDelete = mainPathApi + 'CmsFileContent/delete';
                if (isFolder) {
                    prompMessage = "آیا می خواهید این پوشه و تمامی محتوایات آن حذف شود؟";
                    urlViewModel = mainPathApi + 'CmsFileCategory/getviewmodel';
                    urlDelete = mainPathApi + 'CmsFileCategory/delete';
                }
            ...
            config.getCategoryList = function (filterModel, id) {
                ...
                if (id != undefined) {
                    ...
                    if (filterModel.Filters == undefined && filterModel != undefined)
                        filterModel.Filters = [];
                    filterModel.Filters.push({ PropertyName: 'LinkParentId', SearchType: 0, IntValue1: id });
                }
                else {
                    if (filterModel.Filters == undefined && filterModel != undefined)
                        filterModel.Filters = [];
                    filterModel.Filters.push({ PropertyName: 'LinkParentId', IntValueForceNullSearch: true });
                }
                if (config.patternSearch == undefined)
                    config.patternSearch = '';
                config.currentPosition = [];
                config.currentPosition.push({ Id: null, Title: 'Home' });
                var parentList = [];
                while (node != null && node != undefined) {
                    parentList.push({ Id: node.Id, Title: node.Title });
                    node = node.virtual_Category;
                }
                for (var i = parentList.length - 1; i >= 0; i--)
                    config.currentPosition.push({ Id: parentList[i].Id, Title: parentList[i].Title });


                config.BusyIndicator.isActive = true;
                ajax.call(mainPathApi + "CmsFileCategory/getall", filterModel, 'Post').success(function (response) {
                    config.categoryList = [];
                    config.FileList = []
                    if (id != undefined) {
                        config.categoryList.push({ Title: '', Id: parentId, isParent: true });
                    }
                    ...
                    console.log(data);
                    config.currentPosition = [];
                    config.currentPosition.push({ Id: -1, Title: data.ErrorMessage, isRetry: true });
                    config.BusyIndicator.isActive = false;
                });
            }
            ...
            config.getFileList = function (categoryid) {
                ...
                if (categoryid == undefined || categoryid == null)
                    engine.Filters.push({ PropertyName: 'LinkCategoryId', IntValueForceNullSearch: true });
                else
                    engine.Filters.push({ PropertyName: 'LinkCategoryId', IntValue1: categoryid });

                ajax.call(mainPathApi + "CmsFileContent/GetAll", engine, 'POST').success(function (response) {
            ...
            config.addNecessaryObjectsToFolder = function (folder) {
                ...
                folder.busyIndicator = { isActive: false };
            }

            config.addNecessaryObjectsToFile = function (file) {
                file.busyIndicator = { isActive: false };
            ...
            config.downloadFile = function () {
                ...
                config.BusyIndicator.isActive = true;
                ajax.call("mvc/CmsFileContent/DownloadAllFiles", model, 'POST').success(function (response) {
                    //console.log(response);      
                    window.open('/mvc/CmsFileContent/WhenDownloadCompleted?fileName=' + response.FileName + "&ticket=" + config.Token, '_blank', '');
            ...
            config.pasteFile = function () {
                ...
                config.BusyIndicator.isActive = true;
                ajax.call(mainPathApi + "CmsFileContent/CopyCutFile", model, 'POST').success(function (response) {
                    if (!response.IsSuccess)
            ...
            config.selectFilesToCopy = function (isCut) {
                ...
                for (var i = 0; i < config.folderFileSelected.length; i++) {
                    if (config.folderFileSelected[i].isSelected) {
                        config.selectedFilesToCopyCut.push({ file: config.folderFileSelected[i], isCut: isCut });
                    }
                }
            }
            ...
            config.renameCompleted = function (fileFolder) {
                ...
                var urlViewModel = mainPathApi + 'CmsFileCategory/getviewmodel';
                var urlEdit = mainPathApi + 'CmsFileCategory/edit';
                if (!fileFolder.isFolder) {
                    urlViewModel = mainPathApi + 'CmsFileContent/getviewmodel';
                    urlEdit = mainPathApi + 'CmsFileContent/edit';
            ...
            config.makeNewFolder = function () {
                ...
                ajax.call(mainPathApi + "CmsFileCategory/getviewmodel", "0", 'GET').success(function (response) {
                    if (response.IsSuccess) {
                        //config.BusyIndicator.isActive=true;
                        response.Item.Title = folderName;
                        response.Item.LinkParentId = config.lastCategoryId;
                        ajax.call(mainPathApi + "CmsFileCategory/add", response.Item, 'Post').success(function (response2) {
                            if (response2.IsSuccess == true) {
                                //config.BusyIndicator.isActive=false;
    ...
    return {
        ...
        link: function (scope, element) {
            ...
            if (config.center == undefined)
                config.center = { lat: 33.437039, lng: 53.073182 };
            if (config.center.lat == undefined)
                config.center.lat = 33.437039;
            if (config.center.lng == undefined)
                config.center.lng = 53.073182;
            ...
            if (window['mapEvent' + mapId].positionChanged == undefined)
                window['mapEvent' + mapId].positionChanged = function (latlang, lat, lng) {
                    if (config.scope.selectedItem != undefined && config.locationMemberString != undefined) {
                        config.scope.selectedItem[config.locationMemberString] = "POINT(" + lat + " " + lng + ")";
                    }
            ...
            if (window['mapEvent' + mapId].getPosition == undefined)
                window['mapEvent' + mapId].getPosition = function () {
                    var text = '';
                    if (config.scope.selectedItem != undefined) {
                        if (config.scope.selectedItem[config.locationMember] != undefined) {
                            if (config.scope.selectedItem[config.locationMember].Geography != undefined) {
                                text = config.scope.selectedItem[config.locationMember].Geography.WellKnownText;
                                if (text != undefined && text != null && text != '') {
                                    text = text.replace("POINT (", "").replace("POINT(", "").replace(")", "");
                                    var split = text.split(' ');
                                    config.scope.selectedItem[config.locationMemberString] = "POINT(" + parseFloat(split[0]) + " " + parseFloat(split[1]) + ")";
...
function rashaThumbnail($compile, rashaErManage) {
    return {
        ...
        link: function (scope, element, attrs, ngModel) {
            ...
            var srcThumbnail = '{{infoDomainAddress}}mvc/CmsFileContent/Thumbnail/' + imageId + '?Authorization=' + encodeURIComponent(localStorage.getItem('userGlobaltoken'));
