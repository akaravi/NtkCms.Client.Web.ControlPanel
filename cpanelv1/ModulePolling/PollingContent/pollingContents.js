app.controller("pollingContentCtrl", ["$scope", "$http", "ajax", "rashaErManage", "$modal", "$modalStack", "SweetAlert", "$timeout", "$state", '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $state, $window, $filter) {
 ...
 pollingContent.filePickerMainImage = {
    ...
    pollingContent.ExpireDate = {
        defaultDate: date,
        maxDate: "9017-11-01 23:59:59.997"
    }
...
pollingContent.LinkCategoryIdSelector = {
    ...
    pollingContent.gridOptions = {
        columns: [
            ...
            { name: 'Question', displayName: 'سؤال', sortable: true, type: 'string', visible: 'true' },
            { name: 'FromDate', displayName: 'تاریخ آغاز', sortable: true, isDate: true, type: 'date', visible: 'true' },
            { name: 'ExpireDate', displayName: 'تاریخ پایان', sortable: true, isDate: true, type: 'date', visible: 'true' },
            { name: 'ActionButtons', displayName: 'عملیات', sortable: true, type: 'string', visible: 'true', displayForce: true, template: '<a type="button" ng-show="pollingContent.gridOptions.resultAccess.AccessAddRow || pollingContent.gridOptions.resultAccess.AccessEditRow" class="btn btn-success" ng-click="pollingContent.moveToOptions()"><i class="fa fa-pencil" aria-hidden="true"></i>&nbsp;گزینه ها</a>' }
        ],
        data: {},
        multiSelect: false,
        advancedSearchData: {
    ...
    pollingContent.gridOptions2 = {
        columns: [
            ...
            { name: 'CreatedDate', displayName: 'ساخت', sortable: true, isDate: true, type: 'date', visible: 'true' },
            { name: 'UpdatedDate', displayName: 'ویرایش', sortable: true, isDate: true, type: 'date', visible: 'true' },
            { name: 'Option', displayName: 'گزینه', sortable: true, type: 'string', visible: 'true' },
            { name: 'NumberOfVotes', displayName: 'تعداد', sortable: true, type: 'string', visible: 'true' },
            { name: 'Percentage', displayName: 'درصد', sortable: true, type: 'string', visible: 'true', displayForce: true }
        ],
        data: {},
        multiSelect: false,
        advancedSearchData: {
    ...
    pollingContent.init = function () {
        ...
        pollingContent.categoryBusyIndicator.isActive = true;
        ajax.call(mainPathApi+"pollingContent/getall", {}, "POST").success(function (response) {
    ...
    pollingContent.gridOptions.onRowSelected = function () {
        ...
        filterModel.Filters.push({ PropertyName: "LinkPollingContentId", IntValue1: item.Id, SearchType: 0 });
        pollingContent.addRequested = true;
        pollingContent.optionsBusyIndicator.isActive = true;
        ajax.call(mainPathApi+"pollingoption/getall", filterModel, "POST").success(function (response) {
            pollingContent.addRequested = false;
    ...
    pollingContent.openEditCategoryModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+"pollingCategory/getviewmodel", pollingContent.treeConfig.currentNode.Id, "GET").success(function (response) {
            buttonIsPressed = false;
            ...
            ajax.call(mainPathApi+"CmsFileCategory/getAll", filterModelParentRootFolders, 'POST').success(function (response1) { //Get root directories
                ...
                ajax.call(mainPathApi+"CmsFileContent/GetFilesFromCategory", filterModelRootFiles, 'POST').success(function (response2) { //Get files in root
                    ...
                    if (pollingContent.selectedItem.LinkMainImageId > 0)
                        pollingContent.onSelection({ Id: pollingContent.selectedItem.LinkMainImageId }, true);
                    $modal.open({
    ...
    pollingContent.Showstatistics = function () {
        ...
        ajax.call(mainPathApi+'pollingContent/getviewmodel', pollingContent.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
    ...
    pollingContent.addNewCategory = function (frm) {
        ...
        ajax.call(mainPathApi+"pollingCategory/add", pollingContent.selectedItem, "POST").success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    pollingContent.editCategory = function (frm) {
        ...
        ajax.call(mainPathApi+"pollingCategory/edit", pollingContent.selectedItem, "PUT").success(function (response) {
            pollingContent.addRequested = true;
            //pollingContent.showbusy = false;
    ...
    pollingContent.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                pollingContent.categoryBusyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(mainPathApi+"pollingCategory/getviewmodel", node.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    pollingContent.selectedItemForDelete = response.Item;
                    ajax.call(mainPathApi+"pollingCategory/delete", pollingContent.selectedItemForDelete, "DELETE").success(function (res) {
                        if (res.IsSuccess) {
                            pollingContent.categoryBusyIndicator.isActive = false;
                            if (pollingContent.selectedItemForDelete.LinkParentId == null) {
                                var elementPos = pollingContent.treeConfig.Items.map(function (x) { return x.Id; }).indexOf(pollingContent.selectedItemForDelete.Id); // find the index of an item in an array
    ...
    pollingContent.selectContent = function (node) {
        ...
        ajax.call(mainPathApi+"pollingContent/getall", pollingContent.gridOptions.advancedSearchData.engine, "POST").success(function (response) {
            rashaErManage.checkAction(response);
    ...
    pollingContent.openAddModal = function () {
       ...
       pollingContent.attachedFiles = [];
        ...
        ajax.call(mainPathApi+"pollingcontent/getviewmodel", "0", "GET").success(function (response) {
            rashaErManage.checkAction(response);
            pollingContent.selectedItem = response.Item;
    ...
    pollingContent.openEditModal = function () {
        ...
        ajax.call(mainPathApi+"pollingcontent/getviewmodel", pollingContent.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
            rashaErManage.checkAction(response);
            pollingContent.selectedItem = response.Item;
...
if (response.Item.LinkMainImageId != null) {
                ajax.call(mainPathApi+'CmsFileContent/getviewmodel', response.Item.LinkMainImageId, 'GET').success(function (response2) {
                    buttonIsPressed = false;
  ...
  pollingContent.parseFileIds(response.Item.LinkFileIds);
    ...
    pollingContent.addNewContent = function (frm) {
        ...
        pollingContent.selectedItem.ContentTags = [43, 48, 51];
        pollingContent.addRequested = true;
        ajax.call(mainPathApi+"pollingcontent/add", pollingContent.selectedItem, "POST").success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    pollingContent.editContent = function (frm) {
        ...
        pollingContent.addRequested = true;
        ajax.call(mainPathApi+"pollingcontent/edit", pollingContent.selectedItem, "PUT").success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(mainPathApi+"pollingContent/getviewmodel", pollingContent.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    rashaErManage.checkAction(response);
                    pollingContent.selectedItemForDelete = response.Item;
                    ajax.call(mainPathApi+"pollingContent/delete", pollingContent.selectedItemForDelete, "DELETE").success(function (res) {
 ...
 pollingContent.deleteAttachedFile = function (index) {
    ...
    pollingContent.showUpload = function () { $("#fastUpload").fadeToggle(); }
    ...
    pollingContent.updateContentOption = function (optionIndex) {
        if ($("#option" + optionIndex).is('[readonly]')) { // Update is pressed
            ...
            ajax.call(mainPathApi+"pollingOption/getviewmodel", pollingContent.OptionList[optionIndex].Id, "GET").success(function (response) {
                rashaErManage.checkAction(response);
                ...
                ajax.call(mainPathApi+"pollingOption/edit", pollingContent.selectedItemForUpdate, "PUT").success(function (res) {
                    rashaErManage.checkAction(res);
                }).error(function (data2, errCode2, c2, d2) {
                    rashaErManage.checkAction(data2);
                    pollingContent.treeConfig.showbusy = false;
    ...
    pollingContent.openOptionModal = function (selectedId) {
        ...
        pollingContent.OptionList = [];
        var filterModel = { Filters: [] };
        filterModel.Filters.push({ PropertyName: "LinkPollingContentId", IntValue1: parseInt(selectedId), SearchType: 0 });
        ajax.call(mainPathApi+"pollingoption/getall", filterModel, "POST").success(function (response) {
            rashaErManage.checkAction(response);
            pollingContent.OptionList = response.ListItems;
            pollingContent.calculatePercantage(pollingContent.OptionList);
    ...
    pollingContent.editDeleteOptions = function (frm) {
        ...
        for (var i = 0; i < pollingContent.listComments.length; i++) {
            ajax.call(mainPathApi+"pollingOption/getviewmodel", pollingContent.listComments[i].Id, "GET").success(function (response) {
                pollingContent.showbusy = false;
                pollingContent.showIsBusy = false;
                rashaErManage.checkAction(response);
                pollingContent.selectedItemForDelete = response.Item;
                ajax.call(mainPathApi+"pollingOption/delete", pollingContent.selectedItemForDelete, "DELETE").success(function (res) {
                    pollingContent.treeConfig.showbusy = false;
                    pollingContent.showIsBusy = false;
                    rashaErManage.checkAction(res);
                    if (res.IsSuccess) {
        ...
        pollingContent.addRequested = true;
        for (var i = 0; i < pollingContent.OptionList.length; i++) {
            ajax.call(mainPathApi+'pollingOption/add', pollingContent.OptionList[i], 'POST').success(function (response) {
                pollingContent.addRequested = false;
                pollingContent.contentBusyIndicator.isActive = false;
                rashaErManage.checkAction(response);
                if (response.IsSuccess) {
    ...
    pollingContent.addNewOption = function (frm, addtoGrid) {
        ...
        if (pollingContent.selectedItem.Option == null || pollingContent.selectedItem.Option == "") {
            rashaErManage.showMessage($filter('translate')('Option_can_not_be_empty'));
            return;
        }
        pollingContent.addRequested = true;
        ajax.call(mainPathApi+"pollingoption/add", pollingContent.selectedItem, "POST").success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                if (addtoGrid) {
    ...
    pollingContent.openEditOptionModal = function () {
        ...
        ajax.call(mainPathApi+"pollingoption/getviewmodel", pollingContent.gridOptions2.selectedRow.item.Id, "GET").success(function (response) {
            rashaErManage.checkAction(response);
            pollingContent.selectedItem = response.Item;
            pollingContent.FromDate.defaultDate = pollingContent.selectedItem.FromDate;
            pollingContent.ExpireDate.defaultDate = pollingContent.selectedItem.ExpireDate;
    ...
    pollingContent.editOption = function (frm) {
        ...
        pollingContent.addRequested = true;
        ajax.call(mainPathApi+"pollingoption/edit", pollingContent.selectedItem, "PUT").success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    pollingContent.deleteContentOption = function (index) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), "آیا می خواهید این گزینه را حذف کنید؟", function (isConfirmed) {
            if (isConfirmed) {
                pollingContent.addOptionBusyIndicator = true;
                ajax.call(mainPathApi+"pollingoption/getviewmodel", pollingContent.OptionList[index].Id, "GET").success(function (response) {
 ...
 // ----------- FilePicker Codes --------------------------------
    ...
    pollingContent.deleteAttachedfieldName = function (index) {
        ajax.call(mainPathApi+'pollingContent/delete', pollingContent.contractsList[index], 'DELETE').success(function (res) {
            rashaErManage.checkAction(res);
            if (res.IsSuccess) {
            ...
            $.each(fileIds, function (index, item) {
                if (item == parseInt(item, 10)) {  // Check if item is an integer
                    ajax.call(mainPathApi+'CmsFileContent/getviewmodel', parseInt(item), 'GET').success(function (response) {
                        if (response.IsSuccess) {
                            pollingContent.attachedFiles.push({ fileId: response.Item.Id, filename: response.Item.FileName });
                        }
                    }).error(function (data, errCode, c, d) {
                        rashaErManage.checkAction(data, errCode);
                    });
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            //--------------------------------
                ...
                ajax.call(mainPathApi+"CmsFileContent/getviewmodel", "0", 'GET').success(function (response) {
                    ...
                    var result = 0;
                    ajax.call(mainPathApi+"CmsFileContent/add", pollingContent.FileItem, 'POST').success(function (response) {
                        if (response.IsSuccess) {
                            pollingContent.FileItem = response.Item;
                            pollingContent.showSuccessIcon();
                            $("#save-icon" + index).removeClass("fa-save");
    ...
    pollingContent.exportFile = function () {
        ...
        if (pollingContent.exportOptions) {
            var engine = { SortColumn: "NumberOfVotes", SortType: 0, Filters: [{ PropertyName: "LinkPollingContentId", SearchType: 0, IntValue1: pollingContent.gridOptions.selectedRow.item.Id }], ExportFile: pollingContent.ExportFileClass, CurrentPageNumber: 1 };
            ajax.call(mainPathApi+'pollingoption/exportfile', engine, 'POST').success(function (response) {
                pollingContent.addRequested = false;
                rashaErManage.checkAction(response);
            ...
            ajax.call(mainPathApi+'pollingContent/exportfile', pollingContent.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
                pollingContent.addRequested = false;
                rashaErManage.checkAction(response);
    ...
    pollingContent.toggleExportForm = function (options) {
        ...
        pollingContent.ExportFileClass = {
            FileType: 1, RecieveMethod: 0, RowCount: 200
        };
        $modal.open({
    ...
    pollingContent.getCount = function () {
        ajax.call(mainPathApi+"pollingContent/count", pollingContent.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    pollingContent.getOptionsCount = function () {
        var filterOptions = { Filters: [{ PropertyName: "LinkPollingContentId", SearchType: 0, IntValue1: pollingContent.gridOptions.selectedRow.item.Id }] };
        ajax.call(mainPathApi+"pollingoption/count", filterOptions, 'POST').success(function (response) {
            pollingContent.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    pollingContent.onSelection = function (node, selected) {
        ...
        pollingContent.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            pollingContent.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
