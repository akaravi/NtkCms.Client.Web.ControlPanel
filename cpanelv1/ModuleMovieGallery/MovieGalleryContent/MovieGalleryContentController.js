    ...
    mvGallery.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+"movieGallerycontent/DeleteBatch", deleteFilterModel, "DELETE").success(function (res) {
...
//#help similar & otherinfo
   ...
   mvGallery.editOtherInfo = function(y) {
    ...
    mvGallery.onSelection = function (node, selected) {
        ...
        mvGallery.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            mvGallery.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
