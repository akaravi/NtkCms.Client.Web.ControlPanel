...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 productTag.LinkCategoryTagIdSelector = {
    ...
    productTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", productTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    productTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", productTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    productTag.onSelection = function (node, selected) {
        ...
        productTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            productTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
