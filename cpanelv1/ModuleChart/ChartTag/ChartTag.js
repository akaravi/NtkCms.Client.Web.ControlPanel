...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 chartTag.LinkCategoryTagIdSelector = {
    ...
    chartTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", chartTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    chartTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", chartTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    chartTag.onSelection = function (node, selected) {
        ...
        chartTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            chartTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
