app.controller("chartContentCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window','$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window,$stateParams, $filter) {
    ...
    chartContent.startDate = {
        defaultDate: date
    }
    chartContent.endDate = {
        defaultDate: date
    }
...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 chartContent.LinkCategoryIdSelector = {
    ...
    //#help//add city
    //chartContent.onProvinceChange = function (provinceId) {
    //    chartContent.cityCmsLocatinoListItems = [];//Clear previous cities
    //    chartContent.neighbourhoodCmsLocatinoListItems = [];//Clear previous neighbourhoods
    //    var engine = {
    //    };
    //    var cityfilterModel = {
    //        PropertyName: "LinkParentId",
    ...
    //    chartContent.isloadind = true;
    //    ajax.call(mainPathApi+"CmsLocation/GetAllCities", engine, 'POST').success(function (response) {
    //        chartContent.isloadind = false;
    //        rashaErManage.checkAction(response);
    //        chartContent.cityCmsLocatinoListItems = response.ListItems;
    //    }).error(function (data, errCode, c, d) {
    //        chartContent.busyIndicator.isActive = false;
    //        rashaErManage.checkAction(data, errCode);
    //    });
    //}
    //chartContent.onCitiesChange = function (cityId) {
    //    var engine = {
    //    };
    ...
    //    chartContent.isNeighloadind = true;
    //    ajax.call(mainPathApi+"CmsLocation/GetAllNeighbourhoods", engine, 'POST').success(function (response) {
    //        chartContent.isNeighloadind = false;
    //        rashaErManage.checkAction(response);
    //        chartContent.neighbourhoodCmsLocatinoListItems = response.ListItems;
    //    }).error(function (data, errCode, c, d) {
    //        chartContent.busyIndicator.isActive = false;
    //        rashaErManage.checkAction(data, errCode);
    //    });
    ...
    chartContent.selectContent = function (node) {
        ...
        if(node !=null && node != undefined)
        {
            ...
            chartContent.attachedFiles = null;
            chartContent.attachedFiles = [];
            var s = {
    ...
    chartContent.addNewContentModel = function () {
        ...
        var node = chartContent.treeConfig.currentNode;
        if (node.Id == 0 || !node.Id) {
            rashaErManage.showMessage($filter('translate')('To_Add_A_Chart_Please_Select_The_Category'));
            return;
        }
        chartContent.attachedFiles = [];
        chartContent.attachedFile = "";
...
//---------------Upload Modal Podcast-------------------------------
    ...
    chartContent.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", chartContent.fileIdToDelete, 'GET').success(function (response1) {
    ...
    chartContent.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", chartContent.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
...
//upload file Podcast
 ...
 // replace the file
    ...
    chartContent.onSelection = function (node, selected) {
        ...
        chartContent.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            chartContent.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
