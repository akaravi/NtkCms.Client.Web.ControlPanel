app.controller("linkManagementTargetController", ["$scope", "$stateParams", "$state", "$rootScope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$filter', function ($scope, $stateParams, $state, $rootScope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $filter) {
    ...
    linkManagementTarget.addNewContentModel = function () {
        ...
        var node = linkManagementTarget.treeConfig.currentNode;
        if (node.Id == 0 || !node.Id) {
            rashaErManage.showMessage($filter('translate')('To_Add_A_Link_Management_Please_Select_The_Category'));
            return;
        }
        linkManagementTarget.attachedFiles = [];
        linkManagementTarget.attachedFile = "";
    ...
    linkManagementTarget.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", linkManagementTarget.fileIdToDelete, 'GET').success(function (response1) {
    ...
    linkManagementTarget.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", linkManagementTarget.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    linkManagementTarget.onSelection = function (node, selected) {
        ...
        linkManagementTarget.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            linkManagementTarget.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
