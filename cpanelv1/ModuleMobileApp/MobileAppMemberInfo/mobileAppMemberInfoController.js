app.controller("memberInfoController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $state, $window, $filter) {
    ...
    memberInfo.init = function () {
        ...
        ajax.call(mainPathApi+"MobileAppmemberinfo/getall", memberInfo.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    memberInfo.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'MobileAppmemberInfo/getviewmodel', "0", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            memberInfo.busyIndicator.isActive = false;
            memberInfo.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppMemberInfo/add.html',
                scope: $scope
            });
    ...
    memberInfo.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'MobileAppMemberInfo/add', memberInfo.selectedItem, 'POST').success(function (response) {
            memberInfo.addRequested = false;
            memberInfo.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    memberInfo.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'MobileAppMemberInfo/getviewmodel', memberInfo.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;

            rashaErManage.checkAction(response);
            memberInfo.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppMemberInfo/edit.html',
                scope: $scope
            });
    ...
    memberInfo.editRow = function (frm) {
        ...
        memberInfo.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'MobileAppMemberInfo/edit', memberInfo.selectedItem, 'PUT').success(function (response) {
            memberInfo.addRequested = true;
            rashaErManage.checkAction(response);
            memberInfo.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'MobileAppMemberInfo/getviewmodel', memberInfo.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'MobileAppMemberInfo/delete', memberInfo.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    memberInfo.gridOptions = {
        columns: [
            ...
            { name: 'NotificationId', displayName: 'شناسه PN', sortable: true, type: 'string', visible: true },
    ...
    memberInfo.exportFile = function () {
        ...
        ajax.call(mainPathApi+'MobileAppMemberInfo/exportfile', memberInfo.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            memberInfo.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    memberInfo.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppMemberInfo/report.html',
            scope: $scope
        });
    }
    ...
    memberInfo.getCount = function () {
        ajax.call(mainPathApi+"MobileAppMemberInfo/count", memberInfo.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            memberInfo.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    memberInfo.openSendToModal = function (selectedIndex, selected) {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppMemberInfo/sendNotificationModal.html',
            scope: $scope
        });
    }
    ...
    memberInfo.sendMessageToAll = function (selectedIndex, selectedId) {
        if (memberInfo.selectedItem.TitleMessage == '') {
            rashaErManage.showMessage("عنوان پیام را وارد کنید");
            return;
        }
        if (memberInfo.selectedItem.BodyMessage == '') {
        ...
        ajax.call(mainPathApi+'MobileAppLogNotification/SendNotification', memberInfo.selectedItem, 'POST').success(function (response) {
            memberInfo.busyIndicator.isActive = false;
            memberInfo.addRequested = false;
            memberInfo.sendButtonText = "ارسال";
