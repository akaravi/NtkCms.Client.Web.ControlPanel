app.controller("themeConfigController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$builder', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $builder, $window, $filter) {
    ...
    themeConfig.init = function () {
        ...
        ajax.call(mainPathApi+"mobileappsource/getall", {}, 'POST').success(function (response) {
            ...
            //
            ajax.call(mainPathApi+"mobileappThemeConfig/getAll", themeConfig.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    themeConfig.gridOptions = {
        columns: [
            ...
            { name: 'virtual_Source.Title', displayName: 'منبع', sortable: true, type: 'string' }
        ],
        data: {},
        multiSelect: false,
        advancedSearchData: {
    ...
    themeConfig.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'MobileAppThemeConfig/getviewmodel', "0", 'GET').success(function (response) {
            console.log(response);
            rashaErManage.checkAction(response);
            themeConfig.selectedItem = response.Item;
            $modal.open({
                templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppThemeConfig/add.html',
                scope: $scope
            });
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    themeConfig.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'MobileAppThemeConfig/getviewmodel', themeConfig.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            themeConfig.selectedItem = response.Item;
            ...
            $modal.open({
                templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppThemeConfig/edit.html',
                scope: $scope
            });
            themeConfig.onSourceChange(themeConfig.selectedItem.LinkSourceId);
    ...
    themeConfig.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'MobileAppThemeConfig/add', themeConfig.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    themeConfig.editRow = function (frm) {
        ...
        themeConfig.addRequested = true;
        //themeConfig.saveSubmitValues();
        themeConfig.selectedItem[themeConfig.selectedConfigBuilder] = $.trim(angular.toJson(themeConfig.submitBuilderValue));
        themeConfig.selectedItem[themeConfig.selectedConfigRuntime] = $.trim(angular.toJson(themeConfig.submitRuntimeValue));
        ajax.call(mainPathApi+'mobileAppThemeConfig/edit', themeConfig.selectedItem, 'PUT').success(function (response) {
            themeConfig.addRequested = false;
            themeConfig.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+"mobileappthemeconfig/getviewmodel", themeConfig.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    ...
                    themeConfig.busyIndicator.isActive = true;
                    ajax.call(mainPathApi+"mobileappthemeconfig/delete", themeConfig.selectedItemForDelete, "DELETE").success(function (res) {
                        themeConfig.busyIndicator.isActive = false;
    ...
    themeConfig.openUserBuildSettings = function (id) {
        themeConfig.selectedConfigBuilder = "ThemeConfigBuilderJsonValues";
        themeConfig.selectedConfigRuntime = "ThemeConfigRuntimeJsonValues";
        themeConfig.defaultBuilderValue = [];
        themeConfig.defaultRuntimeValue = [];
        themeConfig.addRequested = true;
        $builder.removeAllFormObject('defaultBuilder');
        $builder.removeAllFormObject('defaultRuntime');
        ajax.call(mainPathApi+"mobileappthemeconfig/getone", { Filters: [{ PropertyName: "LinkSourceId", IntValue1: id }] }, "POST").success(function (response) {
            themeConfig.addRequested = false;
            if (response.IsSuccess) {
                themeConfig.selectedItem.ThemeConfigBuilderFormFormatter = response.Item.ThemeConfigBuilderFormFormatter;

                //Builder
                var customizeValueBuilder = response.Item.ThemeConfigBuilderFormFormatter;
                if (customizeValueBuilder != null && customizeValueBuilder.length > 0) {
                    $.each(customizeValueBuilder, function (i, item) {
                        if (item.FieldName != undefined && item.FieldName != null && item.FieldName != "") {
                            var fieldType = "";
                            if (item.FieldType == "System.Boolean") {
                                fieldType = "radio";
                                $builder.addFormObject('defaultBuilder', {
                                    "component": fieldType,
                            ...
                            if (themeConfig.selectedItem[themeConfig.selectedConfigBuilder] != null && themeConfig.selectedItem[themeConfig.selectedConfigBuilder] != "") {
                                values = $.parseJSON(themeConfig.selectedItem[themeConfig.selectedConfigBuilder]);
                                $.each(values, function (iValue, itemValue) {
                                    if (item.FieldName == itemValue.fieldname) {
                                        $builder.forms.defaultBuilder[i].id = i;
                                        themeConfig.defaultBuilderValue[i] = itemValue.value;
                                    }
                                });
                            }
                        }
                ...
                //Builder
                //Runtime
                var customizeValueRuntime = response.Item.ThemeConfigRuntimeFormFormatter;
                if (customizeValueRuntime != null && customizeValueRuntime.length > 0) {
                    $.each(customizeValueRuntime, function (i, item) {
                        if (item.FieldName != undefined && item.FieldName != null && item.FieldName != "") {
                            var fieldType = "";
                            if (item.FieldType == "System.Boolean") {
                                fieldType = "radio";
                                $builder.addFormObject('defaultRuntime', {
                                    "component": fieldType,
                                    "label": item.FieldTitle,
                                    "description": item.FieldDescription,
                                    "fieldname": item.FieldName,
                            ...
                            //تخصیص مقادیر با تشخصیص نام متغییر
                            if (themeConfig.selectedItem[themeConfig.selectedConfigRuntime] != null && themeConfig.selectedItem[themeConfig.selectedConfigRuntime] != "") {
                                values = $.parseJSON(themeConfig.selectedItem[themeConfig.selectedConfigRuntime]);
                                $.each(values, function (iValue, itemValue) {
                                    if (item.FieldName == itemValue.fieldname) {
                                        $builder.forms.defaultRuntime[i].id = i;
                                        themeConfig.defaultRuntimeValue[i] = itemValue.value;
                                    }
                                });
                            }
                        }
    ...
    themeConfig.loadValues = function (sourceId) {
        //تخصیص مقادیر با تشخصیص نام متغییر
        themeConfig.defaultBuilderValue = [];
        themeConfig.defaultRuntimeValue = [];
        if (themeConfig.selectedItem[themeConfig.selectedConfigBuilder] != null && themeConfig.selectedItem[themeConfig.selectedConfigBuilder] != "") {
            values = $.parseJSON(themeConfig.selectedItem[themeConfig.selectedConfigBuilder]);
            var customizeValueBuilder = themeConfig.selectedItem.ThemeConfigBuilderFormFormatter;
            if (customizeValueBuilder != null && customizeValueBuilder.length > 0) {
                $.each(customizeValueBuilder, function (i, item) {
                    if (item.FieldName != undefined && item.FieldName != null && item.FieldName != "") {
                        $.each(values, function (iValue, itemValue) {
        ...
        if (themeConfig.selectedItem[themeConfig.selectedConfigRuntime] != null && themeConfig.selectedItem[themeConfig.selectedConfigRuntime] != "") {
            values = $.parseJSON(themeConfig.selectedItem[themeConfig.selectedConfigRuntime]);
            var customizeValueRuntime = themeConfig.selectedItem.ThemeConfigRuntimeFormFormatter;
            if (customizeValueRuntime != null && customizeValueRuntime.length > 0) {
                $.each(customizeValueRuntime, function (i, item) {
                    if (item.FieldName != undefined && item.FieldName != null && item.FieldName != "") {
                        $.each(values, function (iValue, itemValue) {
    ...
    themeConfig.onSourceChange = function (sourceId) {
        themeConfig.openUserBuildSettings(sourceId);
    }

    themeConfig.saveSubmitValues = function () {
        themeConfig.addRequested = true;
        themeConfig.busyIndicator.isActive = true;
        themeConfig.selectedItem[themeConfig.selectedConfigBuilder] = $.trim(angular.toJson(themeConfig.submitBuilderValue));
        themeConfig.selectedItem[themeConfig.selectedConfigRuntime] = $.trim(angular.toJson(themeConfig.submitRuntimeValue));
        ajax.call(mainPathApi+'MobilethemeConfig/edit', themeConfig.selectedItem, 'PUT').success(function (response) {
            rashaErManage.checkAction(response);
            themeConfig.addRequested = false;
            themeConfig.busyIndicator.isActive = false;
            themeConfig.closeModal();
            ...
            if (themeConfig.fileIsExist(uploadFile.name)) {
                ...
                ajax
                    ...
                    .success(function (response) {
                        ...
                        var result = 0;
                        ajax
                            .call(mainPathApi + "CmsFileContent/add", themeConfig.FileItem, "POST")
                            .success(function (response) {
                                if (response.IsSuccess) {
                                    themeConfig.FileItem = response.Item;
                                    themeConfig.showSuccessIcon();
    ...
    themeConfig.exportFile = function () {
        ...
        ajax.call(mainPathApi+'mobileappthemeconfig/exportfile', themeConfig.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            themeConfig.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    themeConfig.toggleExportForm = function () {
        ...
        themeConfig.ExportFileClass = { FileType: 1, RecieveMethod: 0, RowCount: 100 };
        $modal.open({
            templateUrl: 'cpanelv1/ModuleMobileApp/MobileAppThemeConfig/report.html',
            scope: $scope
        });
    }
    ...
    themeConfig.getCount = function () {
        ajax.call(mainPathApi+"mobileappthemeconfig/count", themeConfig.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            themeConfig.addRequested = false;
            rashaErManage.checkAction(response);
