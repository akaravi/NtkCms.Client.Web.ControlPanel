app.controller("logNotificationController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $state, $window, $filter) {
    ...
    logNotification.init = function () {
        ...
        ajax.call(mainPathApi+"MobileApplogNotification/getall", logNotification.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    logNotification.exportFile = function () {
        ...
        ajax.call(mainPathApi+'MobileApplogNotification/exportfile', logNotification.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            logNotification.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    logNotification.toggleExportForm = function () {
        ...
        $modal.open({
            templateUrl: 'cpanelv1/ModuleMobileApp/MobileApplogNotification/report.html',
            scope: $scope
        });
    }
    ...
    logNotification.getCount = function () {
        ajax.call(mainPathApi+"MobileApplogNotification/count", logNotification.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            logNotification.addRequested = false;
            rashaErManage.checkAction(response);
