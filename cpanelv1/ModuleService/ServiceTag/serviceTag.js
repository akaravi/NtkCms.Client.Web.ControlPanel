...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 serviceTag.LinkCategoryTagIdSelector = {
    ...
    serviceTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", serviceTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    serviceTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", serviceTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    serviceTag.onSelection = function (node, selected) {
        ...
        serviceTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            serviceTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
