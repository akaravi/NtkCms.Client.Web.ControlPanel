app.controller("inboxCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    var inbox = this;
    inbox.init = function () {
        ajax.call(mainPathApi+"cmsinbox/getall", inbox.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    inbox.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsinbox/getviewmodel', "0", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    inbox.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsinbox/add', inbox.selectedItem, 'POST').success(function (response) {
            inbox.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    inbox.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'cmsinbox/getviewmodel', inbox.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            inbox.selectedItem = response.Item;
            $modal.open({
    ...
    inbox.editRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsinbox/edit', inbox.selectedItem, 'PUT').success(function (response) {
            inbox.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                inbox.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsinbox/getviewmodel', inbox.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsinbox/delete', inbox.selectedItemForDelete, 'DELETE').success(function (res) {
