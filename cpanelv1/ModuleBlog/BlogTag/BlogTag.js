...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 blogTag.LinkCategoryTagIdSelector = {
    ...
    blogTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", blogTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    blogTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", blogTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    blogTag.onSelection = function (node, selected) {
        ...
        blogTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            blogTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
