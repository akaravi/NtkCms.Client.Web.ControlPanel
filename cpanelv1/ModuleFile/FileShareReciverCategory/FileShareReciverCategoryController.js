app.controller("fileShareReciverCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$rootScope', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $rootScope, $filter) {
    ...
    fileShareReciverCategory.init = function () {
        ...
        fileShareReciverCategory.categoryBusyIndicator.isActive = true;
        ajax.call(mainPathApi+"CmsFileShareMainAdminSetting/getall", { RowPerPage: 1000 }, 'POST').success(function (response) {
        ...
        ajax.call(mainPathApi+"CmsFileShareReciverCategory/getall", {}, 'POST').success(function (response) {
    ...
    fileShareReciverCategory.addNewCategoryModel = function () {
        ...
        ajax.call(mainPathApi+'CmsFileShareMainAdminSetting/getviewmodel', "0", 'GET').success(function (response) {
    ...
    fileShareReciverCategory.EditCategoryModel = function () {
        ...
        fileShareReciverCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(mainPathApi+'CmsFileShareMainAdminSetting/getviewmodel', fileShareReciverCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            fileShareReciverCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            fileShareReciverCategory.selectedItem = response.Item;
    ...
    fileShareReciverCategory.addNewCategory = function (frm) {
        ...
        ajax.call(mainPathApi+'CmsFileShareMainAdminSetting/add', fileShareReciverCategory.selectedItem, 'POST').success(function (response) {
            fileShareReciverCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    fileShareReciverCategory.EditCategory = function (frm) {
        ...
        fileShareReciverCategory.categoryBusyIndicator.isActive = true;
        fileShareReciverCategory.addRequested = true;
        ajax.call(mainPathApi+'CmsFileShareMainAdminSetting/edit', fileShareReciverCategory.selectedItem, 'PUT').success(function (response) {
    ...
    fileShareReciverCategory.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                buttonIsPressed = true;
                ajax.call(mainPathApi+'CmsFileShareMainAdminSetting/getviewmodel', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    fileShareReciverCategory.selectedItemForDelete = response.Item;
                    console.log(fileShareReciverCategory.selectedItemForDelete);
                    ajax.call(mainPathApi+'CmsFileShareMainAdminSetting/delete', fileShareReciverCategory.selectedItemForDelete, 'DELETE').success(function (res) {
                        fileShareReciverCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    fileShareReciverCategory.selectContent = function (node) {
        ...
        ajax.call(mainPathApi+"CmsFileShareReciverCategory/getall", fileShareReciverCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            fileShareReciverCategory.contentBusyIndicator.isActive = false;
            fileShareReciverCategory.ListItems = response.ListItems;
            fileShareReciverCategory.gridOptions.fillData(fileShareReciverCategory.ListItems, response.resultAccess); // Sending Access as an argument
    ...
    fileShareReciverCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'CmsFileShareReciverCategory/getviewmodel', "0", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            fileShareReciverCategory.selectedItem = response.Item;
    ...
    fileShareReciverCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'CmsFileShareReciverCategory/getviewmodel', fileShareReciverCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
    ...
    fileShareReciverCategory.addNewContent = function (frm) {
        ...
        ajax.call(mainPathApi+'CmsFileShareReciverCategory/add', fileShareReciverCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            fileShareReciverCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    fileShareReciverCategory.editContent = function (frm) {
        ...
        ajax.call(mainPathApi+'CmsFileShareReciverCategory/edit', fileShareReciverCategory.selectedItem, 'PUT').success(function (response) {
            fileShareReciverCategory.categoryBusyIndicator.isActive = false;
            fileShareReciverCategory.addRequested = false;
            fileShareReciverCategory.treeConfig.showbusy = false;
    ...
    fileShareReciverCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                fileShareReciverCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(mainPathApi+"CmsFileShareReciverCategory/getviewmodel", fileShareReciverCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    fileShareReciverCategory.showbusy = false;
                    fileShareReciverCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    fileShareReciverCategory.selectedItemForDelete = response.Item;
                    console.log(fileShareReciverCategory.selectedItemForDelete);
                    ajax.call(mainPathApi+"CmsFileShareReciverCategory/delete", fileShareReciverCategory.selectedItemForDelete, "DELETE").success(function (res) {
                        fileShareReciverCategory.categoryBusyIndicator.isActive = false;
                        fileShareReciverCategory.treeConfig.showbusy = false;
                        fileShareReciverCategory.showIsBusy = false;
    ...
    fileShareReciverCategory.exportFile = function () {
        ...
        ajax.call(mainPathApi+'CmsFileShareReciverCategory/exportfile', fileShareReciverCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            fileShareReciverCategory.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    fileShareReciverCategory.getCount = function () {
        ajax.call(mainPathApi+"CmsFileShareReciverCategory/count", fileShareReciverCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            fileShareReciverCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    fileShareReciverCategory.onSelection = function (node, selected) {
        ...
        fileShareReciverCategory.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            fileShareReciverCategory.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
