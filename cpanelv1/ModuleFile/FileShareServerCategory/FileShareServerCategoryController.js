app.controller("fileShareServerCategoryController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$window', '$rootScope', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $window, $rootScope, $filter) {
    ...
    fileShareServerCategory.init = function () {
        ...
        ajax.call(mainPathApi+"FileShareServerCategory/getall", {}, 'POST').success(function (response) {
    ...
    fileShareServerCategory.EditCategoryModel = function () {
        ...
        fileShareServerCategory.contentBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(mainPathApi+'FileShareMainAdminSetting/getviewmodel', fileShareServerCategory.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            fileShareServerCategory.contentBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            fileShareServerCategory.selectedItem = response.Item;
    ...
    fileShareServerCategory.addNewCategory = function (frm) {
        ...
        ajax.call(mainPathApi+'FileShareMainAdminSetting/add', fileShareServerCategory.selectedItem, 'POST').success(function (response) {
            fileShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    fileShareServerCategory.EditCategory = function (frm) {
        ...
        fileShareServerCategory.categoryBusyIndicator.isActive = true;
        fileShareServerCategory.addRequested = true;
        ajax.call(mainPathApi+'FileShareMainAdminSetting/edit', fileShareServerCategory.selectedItem, 'PUT').success(function (response) {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'FileShareMainAdminSetting/getviewmodel', node.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'FileShareMainAdminSetting/delete', fileShareServerCategory.selectedItemForDelete, 'DELETE').success(function (res) {
                        fileShareServerCategory.categoryBusyIndicator.isActive = false;
                        if (res.IsSuccess) {
    ...
    fileShareServerCategory.selectContent = function (node) {
        ...
        ajax.call(mainPathApi+"FileShareServerCategory/getall", fileShareServerCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            fileShareServerCategory.contentBusyIndicator.isActive = false;
            fileShareServerCategory.ListItems = response.ListItems;
            fileShareServerCategory.gridOptions.fillData(fileShareServerCategory.ListItems, response.resultAccess); // Sending Access as an argument
    ...
    fileShareServerCategory.addNewContentModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'FileShareServerCategory/getviewmodel', "0", 'GET').success(function (response) {
            buttonIsPressed = false;
            //console.log(response);
            rashaErManage.checkAction(response);
            fileShareServerCategory.selectedItem = response.Item;
    ...
    fileShareServerCategory.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'FileShareServerCategory/getviewmodel', fileShareServerCategory.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
    ...
    fileShareServerCategory.addNewContent = function (frm) {
        ...
        ajax.call(mainPathApi+'FileShareServerCategory/add', fileShareServerCategory.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            fileShareServerCategory.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    fileShareServerCategory.editContent = function (frm) {
        ...
        ajax.call(mainPathApi+'FileShareServerCategory/edit', fileShareServerCategory.selectedItem, 'PUT').success(function (response) {
            fileShareServerCategory.categoryBusyIndicator.isActive = false;
            fileShareServerCategory.addRequested = false;
            fileShareServerCategory.treeConfig.showbusy = false;
    ...
    fileShareServerCategory.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translatentk')('warning')), ($filter('translatentk')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                fileShareServerCategory.showIsBusy = true;
                buttonIsPressed = true;
                ajax.call(mainPathApi+"FileShareServerCategory/getviewmodel", fileShareServerCategory.gridOptions.selectedRow.item.Id, "GET").success(function (response) {
                    buttonIsPressed = false;
                    fileShareServerCategory.showbusy = false;
                    fileShareServerCategory.showIsBusy = false;
                    rashaErManage.checkAction(response);
                    fileShareServerCategory.selectedItemForDelete = response.Item;
                    console.log(fileShareServerCategory.selectedItemForDelete);
                    ajax.call(mainPathApi+"FileShareServerCategory/delete", fileShareServerCategory.selectedItemForDelete, "DELETE").success(function (res) {
                        fileShareServerCategory.categoryBusyIndicator.isActive = false;
                        fileShareServerCategory.treeConfig.showbusy = false;
                        fileShareServerCategory.showIsBusy = false;
   ...
   // End of Upload Modal-----------------------------------------
    ...
    fileShareServerCategory.exportFile = function () {
        ...
        ajax.call(mainPathApi+'FileShareServerCategory/exportfile', fileShareServerCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            fileShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    fileShareServerCategory.getCount = function () {
        ajax.call(mainPathApi+"FileShareServerCategory/count", fileShareServerCategory.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            fileShareServerCategory.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    fileShareServerCategory.onSelection = function (node, selected) {
        ...
        fileShareServerCategory.selectedItem.previewImageSrc = mainPathCmsFiles+"loader.gif";
        ajax.call(mainPathApi+"FileContent/getviewmodel", node.Id, "GET").success(function (response) {
            fileShareServerCategory.selectedItem.previewImageSrc = mainPathRouteUploadFiles + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
