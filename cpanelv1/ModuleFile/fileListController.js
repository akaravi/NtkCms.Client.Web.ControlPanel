app.controller("filesListCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    filesList.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'CmsFileContent/add', filesList.selectedItem, 'POST').success(function (response) {
            filesList.addRequested = false;
            filesList.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    filesList.openViewModal = function () {
        ...
        ajax.call(mainPathApi+'CmsFileContent/getviewmodel', filesList.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            filesList.selectedItem = response.Item;
            $modal.open({
    ...
    //filesList.openEditModal = function () {
    //    filesList.modalTitle = 'ویرایش';
    //    if (!filesList.gridOptions.selectedRow.item) {
    //        rashaErManage.showMessage($filter('translate')('Please_Select_A_Row_To_Edit'));
    //        return;
    ...
    //        filesList.selectedItem = response.Item;
    //        $modal.open({
    //            templateUrl: 'cpanelv1/ModuleFile/edit.html',
    //            scope: $scope
    //        });
    //    }).error(function (data, errCode, c, d) {
    //        rashaErManage.checkAction(data, errCode);
    ...
    filesList.editRow = function (frm) {
        ...
        filesList.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsFileContent/edit', filesList.selectedItem, 'PUT').success(function (response) {
            filesList.addRequested = true;
            rashaErManage.checkAction(response);
            filesList.busyIndicator.isActive = false;
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'CmsFileContent/getviewmodel', filesList.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'CmsFileContent/delete', filesList.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    filesList.exportFile = function () {
        ...
        filesList.addRequested = true;
        ajax.call(mainPathApi+'CmsFileContent/exportfile', filesList.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            filesList.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    filesList.getCount = function () {
        ajax.call(mainPathApi+"CmsFileContent/count", filesList.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            filesList.addRequested = false;
            rashaErManage.checkAction(response);
