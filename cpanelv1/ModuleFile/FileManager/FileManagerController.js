app.controller("FileManager", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$timeout', '$rootScope', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $timeout, $rootScope, $filter) {
    ...
    fdm.init = function () {
        ...
        ajax.call(mainPathApi+"CmsFileCategory/getall", { RowPerPage: 500 }, 'Post').success(function (response) {
    ...
    fdm.uploadByUrlStart = function (index, uploadFile) {
        ...
        // ------- fdm.saveNewFile()  ----------------------
        var result = 0;
        ajax.call(mainPathApi+"CmsFileContent/UploadByUrl", { url: uploadFile.Url}, 'POST').success(function (response) {
            fdm.loadingBusyIndicator.isActive = false;
    ...
    fdm.search = function (input) {
        ...
        //Search in files
        fdm.FileList = [];
        ajax.call(mainPathApi+"CmsFileContent/getall", filterModel, 'POST').success(function (response) {
    ...
    fdm.itemClicked = function ($event, index, type) {
        if (type == 'file' || type == 1) {
            ...
            fdm.msgText = "File name: \"" + fdm.selectedFileName + "\"" + "<br/> file Description: " + fdm.selectedFileDescription + "<br/> file ID: " + fdm.selectedFileId + "\"<br/> file Src: " + $rootScope.tokenInfo.Item.virtual_CmsSite.Title + '/files/' + fdm.selectedFileId + '/' + fdm.selectedFileName + "<br/>";//+"<button ng-click="+fdm.EditFile(fdm.selectedFileId)+">Edit</button>";
            fdm.msgColor = "#007e1e";
        }
        else {
    ...
    */
    ...
    fdm.saveNewFolder = function () {
        fdm.loadingBusyIndicator.isActive = true;
        ajax.call(mainPathApi+"CmsFileCategory/add", fdm.Item, 'Post').success(function (response) {
            fdm.Item = response.Item;
            fdm.loadingBusyIndicator.isActive = false;
    ...
    fdm.deleteFileOrFolder = function () {
        ...
        if (fdm.fileTypes == 1) { // file type
            ajax.call(mainPathApi+"CmsFileContent/getviewmodel", fdm.fileIdToDelete, 'GET').success(function (response) {
                ajax.call(mainPathApi+'CmsFileContent/delete', response.Item, 'DELETE').success(function (response) {
                    fdm.loadingBusyIndicator.isActive = false;
            ...
            ajax.call(mainPathApi+"CmsFileCategory/getviewmodel", fdm.fileIdToDelete, 'GET').success(function (response) {
                ajax.call(mainPathApi+'CmsFileCategory/delete', response.Item, 'DELETE').success(function (response) {
                    if (response.IsSuccess) {
                        fdm.loadingBusyIndicator.isActive = false;
        ...
        if ($("#save-icon" + index).hasClass("fa-save")) {
            ...
            if (fdm.fileIsExist(uploadFile.name)) { // File already exists
                if (confirm('File "' + uploadFile.name + '" already exists! Do you want to replace the new file?')) {
                    ...
                    fdm.loadingBusyIndicator.isActive = true;
                    // Delete the file
                    ajax.call(mainPathApi+"CmsFileContent/getviewmodel", fdm.fileIdToDelete, 'GET').success(function (response1) {
                                ...
                                if (response2.IsSuccess == true) {
                                    ...
                                    ajax.call(mainPathApi+"CmsFileContent/getviewmodel", "0", 'GET').success(function (response3) {
                                        if (response3.IsSuccess == true) {
                                            ...
                                            // ------- fdm.saveNewFile()  ----------------------
                                            var result = 0;
                                            ajax.call(mainPathApi+"CmsFileContent/add", fdm.FileItem, 'POST').success(function (response) {
                                                fdm.loadingBusyIndicator.isActive = false;
                                                if (response.IsSuccess) {
    ...
    fdm.downloadFile = function (Id, FileName) {
        ...
        if (fdm.selectedFileId != undefined && fdm.selectedFileName != undefined) {
            window.open('/files/' + fdm.selectedFileId + '/' + fdm.selectedFileName, '_blank', '');
        }
    }
    ...
    fdm.copyWork = function (id, name, parentFolderDes) {
        if (fdm.CopyType == 1) { // file type
            ...
            fdm.loadingBusyIndicator.isActive = true;
            ajax.call(mainPathApi+"CmsFileContent/getviewmodel", id, 'GET').success(function (response) {
                response.Item.LinkCategoryId = parentFolderDes;
                ajax.call(mainPathApi+'CmsFileContent/Copy', response.Item, 'POST').success(function (response) {
            ...
            fdm.loadingBusyIndicator.isActive = true;
            ajax.call(mainPathApi+"CmsFileCategory/getviewmodel", id, 'GET').success(function (response) {
                response.Item.LinkParentId = parentFolderDes;
                ajax.call(mainPathApi+'CmsFileCategory/add', response.Item, 'POST').success(function (response) {
                    fdm.categoryList.unshift(response.Item);
                    fdm.loadingBusyIndicator.isActive = false;
    ...
    fdm.cutWork = function (id, name, parentFolderDes) {
        if (fdm.CutType == 1) { // file type
            ...
            ajax.call(mainPathApi+"CmsFileContent/getviewmodel", id, 'GET').success(function (response) {
                response.Item.LinkCategoryId = parentFolderDes;
                ajax.call(mainPathApi+'CmsFileContent/edit', response.Item, 'PUT').success(function (response) {
            ...
            ajax.call(mainPathApi+"CmsFileCategory/getviewmodel", id, 'GET').success(function (response) {
                ...
                ajax.call(mainPathApi+'CmsFileCategory/edit', response.Item, 'PUT').success(function (response) {
    ...
    fdm.renameFile = function (newFileName) {
        if (fdm.fileTypes == 1) { // file type
            ajax.call(mainPathApi+"CmsFileContent/getviewmodel", fdm.fileIdToRename, 'GET').success(function (response) {
            ...
            ajax.call(mainPathApi+"CmsFileCategory/getviewmodel", fdm.fileIdToRename, 'GET').success(function (response) {
    ...
    fdm.imageFileEdit = function (Id, FileName) {
        ...
        //window.open('/files/' + fdm.selectedFileId + '/' + fdm.selectedFileName, '_blank', '');
        ajax.call(mainPathApi+"CmsFileContent/imageFileEdit", DownloadModel, 'POST').success(function (response) {
            fdm.loadingBusyIndicator.isActive = false;
            if (response.IsSuccess) {
