app.controller("musicGalleryCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', '$timeout', '$window','$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, $timeout, $window,$stateParams, $filter) {
   ...
   var date = moment().format();
    ...
    mscGallery.fetchImageURLs = function (listItems) {
        ...
        ajax.call(mainPathApi+"CmsFileContent/PreviewThumbnailImages", modelList, "POST").success(function (response2) {
...
mscGallery.LinkCategoryIdSelector = {
    ...
    mscGallery.addFromFolder = function (frm) {
        ...
        mscGallery.addRequested = true;
        ajax.call(mainPathApi+'CmsFileContent/GetFilesFromCategory', mscGallery.selectedItem.FolderId, 'POST').success(function (response) {
            mscGallery.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    mscGallery.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+"musicgallerycontent/deleteBatch", deleteFilterModel, "DELETE").success(function (res) {
...
//#help similar & otherinfo
   ...
   mscGallery.editOtherInfo = function(y) {
    ...
    mscGallery.onSelection = function (node, selected) {
        ...
        mscGallery.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            mscGallery.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
