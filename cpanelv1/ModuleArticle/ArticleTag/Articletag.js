  ...
  //#help similar & otherinfo
   ...
   articleTag.editOtherInfo = function(y) {
    ...
    articleTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", articleTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    articleTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", articleTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    articleTag.onSelection = function (node, selected) {
        ...
        articleTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            articleTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
