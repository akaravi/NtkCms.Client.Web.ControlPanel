...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 shopTag.LinkCategoryTagIdSelector = {
    ...
    shopTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", shopTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    shopTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", shopTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    shopTag.onSelection = function (node, selected) {
        ...
        shopTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            shopTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
