app.controller("memberPropertyController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
 ...
 //#help/ سلکتور نوع عضو 
    ...
    memberProperty.init = function () {
        ...
        ajax.call(mainPathApi+"memberproperty/getAllwithalias", memberProperty.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    memberProperty.loadDetailValues = function (propertyTypeId, propertyId) {
        ...
        ajax.call(mainPathApi+"memberPropertyDetail/GetAll", engine1, 'POST').success(function (response1) {
    ...
    //memberProperty.isNeighloadind = true;

    //memberProperty.onProvinceChange = function (provinceId) {
    //    memberProperty.cityCmsLocatinoListItems = [];//Clear previous cities
    //    memberProperty.neighbourhoodCmsLocatinoListItems = [];//Clear previous neighbourhoods
    //    var engine = {
    //    };
    //    var cityfilterModel = {
    //        PropertyName: "LinkParentId",
    //        IntValue1: parseInt(provinceId),
    //        SearchType: 0
    //    };
    //    engine.Filters = [];
    //    engine.Filters.push(cityfilterModel);
    //    memberProperty.isloadind = true;
    //    ajax.call(mainPathApi+"CmsLocation/GetAllCities", engine, 'POST').success(function (response) {
    //        memberProperty.isloadind = false;
    //        rashaErManage.checkAction(response);
    //        memberProperty.cityCmsLocatinoListItems = response.ListItems;
    //    }).error(function (data, errCode, c, d) {
    //        memberProperty.busyIndicator.isActive = false;
    //        rashaErManage.checkAction(data, errCode);
    //    });
    ...
    //memberProperty.onCitiesChange = function (cityId) {
    //    var engine = {
    //    };
    //    var neighbourhoodfilterModel = {
    //        PropertyName: "LinkParentId",
    //        IntValue1: parseInt(cityId),
    //        SearchType: 0
    //    };
    //    engine.Filters = [];
    //    engine.Filters.push(neighbourhoodfilterModel);
    //    memberProperty.isNeighloadind = true;
    //    ajax.call(mainPathApi+"CmsLocation/GetAllNeighbourhoods", engine, 'POST').success(function (response) {
    //        memberProperty.isNeighloadind = false;
    //        rashaErManage.checkAction(response);
    //        memberProperty.neighbourhoodCmsLocatinoListItems = response.ListItems;
    //    }).error(function (data, errCode, c, d) {
    //        memberProperty.busyIndicator.isActive = false;
    //        rashaErManage.checkAction(data, errCode);
    //    });
   ...
   /* memberProperty.contractsList = [];
    memberProperty.openAddContractModal = function (propertyId, propertyTitle) {
        ajax.call(mainPathApi+"membercontracttype/getall", {}, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            memberProperty.busyIndicator.isActive = false;
            memberProperty.contractTypeListItems = response.ListItems;
            memberProperty.selectedItem.LinkPropertyTypeId = null;
        }).error(function (data, errCode, c, d) {
            memberProperty.busyIndicator.isActive = false;
            rashaErManage.checkAction(data, errCode);
        });
        ajax.call(mainPathApi+'memberContract/getviewmodel', "0", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                memberProperty.selectedItem = response.Item;
                memberProperty.selectedItem.LinkPropertyId = parseInt(propertyId);  // Set LinkPropertyId for new Contract
                memberProperty.selectedItem.LinkPropertyTitle = propertyTitle;  // Set LinkPropertyId for new Contract
                var model = {
                };
                model.Filters = [];
                model.Filters.push({ PropertyName: "LinkPropertyId", IntValue1: parseInt(propertyId), SearchType: 0 });
                ajax.call(mainPathApi+'memberContract/getall', model, 'POST').success(function (response) {
                    rashaErManage.checkAction(response);
                    memberProperty.contractsList = response.ListItems;
                    memberProperty.contractsListresultAccess = response.resultAccess;
                    $modal.open({
                        templateUrl: 'cpanelv1/Modulemember/memberProperty/addContract.html',
                        scope: $scope
                    });
                }).error(function (data, errCode, c, d) {
                    rashaErManage.checkAction(data, errCode);
    ...
    memberProperty.addContract = function () {
        var duplicateContract = false;
        var erMessage = "";
        if (memberProperty.selectedItem.LinkmemberContractTypeId == null || memberProperty.selectedItem.LinkmemberContractTypeId < 0)
            return;
        $.each(memberProperty.contractsList, function (index, item) {
            if (item.LinkmemberContractTypeId == parseInt(memberProperty.selectedItem.LinkmemberContractTypeId)) {
        ...
        memberProperty.addRequested = true;
        memberProperty.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'memberContract/add', memberProperty.selectedItem, 'POST').success(function (response) {
            memberProperty.addRequested = false;
            memberProperty.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                memberProperty.contractsList.push(response.Item);
                memberProperty.selectedItem.SalePrice = null;
                memberProperty.selectedItem.PresalePrice = null;
                memberProperty.selectedItem.DepositPrice = null;
                memberProperty.selectedItem.RentPrice = null;
                memberProperty.selectedItem.Description = "";
            }
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
            memberProperty.addRequested = false;
    ...
    memberProperty.editContract = function (index) {
        memberProperty.addRequested = true;
        ajax.call(mainPathApi+'memberContract/edit', memberProperty.contractsList[index], 'PUT').success(function (res) {
            memberProperty.addRequested = false;
            rashaErManage.checkAction(res);
            if (res.IsSuccess) {
    ...
    memberProperty.deleteContract = function (index) {
        memberProperty.addRequested = true;
        ajax.call(mainPathApi+'memberContract/delete', memberProperty.contractsList[index], 'DELETE').success(function (res) {
            memberProperty.addRequested = false;
            rashaErManage.checkAction(res);
            if (res.IsSuccess) {
                memberProperty.contractsList.splice(index, 1);
    ...
    memberProperty.onRecordStatusChange = function (record) {
        ...
        //ajax.call(mainPathApi+"memberproperty/getAllwithalias", filterstatus, 'POST').success(function (response) {
        //    rashaErManage.checkAction(response);
        //    memberProperty.ListItems = response.ListItems;
        //    memberProperty.gridOptions.fillData(memberProperty.ListItems, response.resultAccess);
        //    memberProperty.gridOptions.currentPageNumber = response.CurrentPageNumber;
