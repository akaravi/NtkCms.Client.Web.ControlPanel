...
memberUser.selectedItemMemberInSite = {};
 ...
 //#help property Grid Options
   /* memberUser.gridOptionsProperty = {
        columns: [
            { name: 'Id', displayName: 'کد سیستمی', sortable: true, type: 'integer', visible: true },
            { name: 'LinkSiteId', displayName: 'کد سیستمی سایت', sortable: true, type: 'integer', visible: true },
            { name: 'CreatedDate', displayName: 'ساخت', sortable: true, isDate: true, type: 'date', visible: 'true' },
            ...
            { name: "ActionButton", displayName: "مقادیر", sortable: true, displayForce: true, template: "<button class=\"btn btn-success\" ng-click=\"memberUser.openAddPropertyModal(x.Id,x.MemberUserId)\" title=\"مقداردهی\" type=\"button\"><i class=\"fa fa-pencil fa-1x\" aria-hidden=\"true\"></i></button>" },
            //{ name: "ActionButton", displayName: "تاریخچه", sortable: true, displayForce: true, template: "<button class=\"btn btn-success\" ng-click=\"memberUser.openAddHistory(x.Id)\" title=\"مقداردهی\" type=\"button\"><i class=\"fa fa-pencil fa-1x\" aria-hidden=\"true\"></i></button>" },
            { name: "ActionButton", displayName: "مشاهده تاریخچه", sortable: true, displayForce: true, template: "<button class=\"btn btn-success\" ng-click=\"memberUser.openPreviewHistory(x.Id)\" title=\"مقداردهی\" type=\"button\"><i class=\"fa fa-pencil fa-1x\" aria-hidden=\"true\"></i></button>" }
        ],
        data: {},
        multiSelect: false,
        showUserSearchPanel: false,
...
/*memberUser.openPreviewHistory = function (PropertyTypeId) {
 var s = {
        PropertyName: "LinkPropertyTypeId",
        IntValue1: PropertyTypeId,
        SearchType: 0
    }
    var engineproperty = {};
    engineproperty.Filters = [];
...
memberUser.showlistHistory=true;
        var s = {
            PropertyName: "LinkPropertyTypeId",
            IntValue1: PropertyTypeId,
            SearchType: 0
        ...
        var engine2 = {};
        engine2.Filters = [];
    ajax.call(mainPathApi+"MemberProperty/getall", engine, 'POST').success(function (response) {
        memberUser.ListPropertyItem = response.ListItems;
        
        angular.forEach(memberUser.ListPropertyItem, function (item, key) {
         var d = {
 ...
 /*ajax.call(mainPathApi+"MemberHistory/getall", engine2, 'POST').success(function (response1) {
            memberUser.listHistorys = response1.ListItems;
            rashaErManage.checkAction(response1);
            memberUser.gridOptionsHistory.fillData(memberUser.listHistorys, response1.resultAccess);
            memberUser.gridOptionsHistory.currentPageNumber = response1.CurrentPageNumber;
            memberUser.gridOptionsHistory.totalRowCount = response1.TotalRowCount;
            memberUser.gridOptionsHistory.RowPerPage = response1.RowPerPage;
    }).error(function (data, errCode, c, d) {
        console.log(data);
    });
}*/
memberUser.onPropertyTypeChange = function (propertyTypeId) {
        ...
        ajax.call(mainPathApi+"memberPropertyDetail/GetAll", engine, 'POST').success(function (response) {
...
// Open Add Modal property
    memberUser.openAddPropertyModal = function (PropertyTypeId,UserId) {
        ...
        ajax.call(mainPathApi+'memberproperty/getviewmodel', "0", 'GET').success(function (response) {
...
// Open Add Modal History
   /* memberUser.openAddModalHistory = function () {
      ...
      //if (buttonIsPressed) return;
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'MemberHistory/getviewmodel', "0", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
...
/* memberUser.addNewContenthistory = function (frm) {
       ...
       /* if (memberUser.selectedItemhistory.LinkPropertyId ==null) {
        ...
        ajax.call(mainPathApi+'MemberHistory/add', memberUser.selectedItemhistory, 'POST').success(function (response) {
            //memberUser.addRequested = false;
            //memberUser.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
...
// Open Edit Modal History
   /* memberUser.openEditModelHistory = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'MemberHistory/getviewmodel', memberUser.gridOptionsHistory.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
...
/* memberUser.editNewContenthistory = function (frm) {
        ...
        //memberUser.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'MemberHistory/edit', memberUser.selectedItemhistory, 'PUT').success(function (response) {
            //memberUser.addRequested = true;
            rashaErManage.checkAction(response);
            //memberUser.busyIndicator.isActive = false;
            if (response.IsSuccess) {
...
/* memberUser.deleteContentHistory = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'MemberHistory/getviewmodel', memberUser.gridOptionsHistory.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'MemberHistory/delete', memberUser.selectedItemForDelete, 'DELETE').success(function (res) {
  ...
  /*  memberUser.addRowValueProperty = function (frm) {
        ...
        ajax.call(mainPathApi+'memberproperty/add', memberUser.selectedItemValue, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                memberUser.closeModal();
                ajax.call(mainPathApi+"memberpropertydetailvalue/getviewmodel", '0', 'GET').success(function (response1) {
                    rashaErManage.checkAction(response1);
                    ...
                    ajax.call(mainPathApi+'memberPropertyDetailValue/AddBatch', memberUser.valueItems, 'POST').success(function (response2) {
...
//memberUser.showlistHistory=false;
   ...
   //#help show list property
   /* memberUser.showlistProperty = function (sItem) {
memberUser.showlistPeoperty=true;
memberUser.showlistFile=false;
//memberUser.showlistHistory=false;
...
angular.forEach(memberUser.treeConfig.Items, function (itemG, key) {
...
{
            ajax.call(mainPathApi+"MemberPropertyType/getall", engine, "POST").success(function (response) {
                memberUser.listComments = response.ListItems;
                angular.forEach( response.ListItems, function (item, key) {
                    item.MemberUserId=MemberUserId;
                });
                rashaErManage.checkAction(response);
                memberUser.gridOptionsProperty.fillData(memberUser.listComments, response.resultAccess);
                memberUser.gridOptionsProperty.currentPageNumber = response.CurrentPageNumber;
                memberUser.gridOptionsProperty.totalRowCount = response.TotalRowCount;
                memberUser.gridOptionsProperty.RowPerPage = response.RowPerPage;
                memberUser.showGridComment = true;
                memberUser.Title = memberUser.gridOptions.selectedRow.item.Title;
            });
            $('html, body').animate({
...
{
memberUser.gridOptionsProperty.fillData({}, {});
}       
    }*/
    ...
    memberUser.init = function () {
        ajax.call(mainPathApi+"memberuser/getAllGender", {}, 'POST').success(function (response) {
            memberUser.Gender = response;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
        memberUser.categoryBusyIndicator.isActive = true;
        ...
        ajax.call(mainPathApi+"memberuser/getall", memberUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    memberUser.addNewCategoryModel = function () {
        ...
        memberUser.addRequested = false;
        buttonIsPressed = true;
        ajax.call(mainPathApi+'MemberGroup/getviewmodel', "0", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
    ...
    memberUser.openEditCategoryModel = function () {
        ...
        memberUser.categoryBusyIndicator.isActive = true;
        buttonIsPressed = true;
        ajax.call(mainPathApi+'MemberGroup/getviewmodel', memberUser.treeConfig.currentNode.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            memberUser.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItem = response.Item;
    ...
    memberUser.addNewCategory = function (frm) {
        ...
        ajax.call(mainPathApi+'MemberGroup/add', memberUser.selectedItem, 'POST').success(function (response) {
            memberUser.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    memberUser.editCategory = function (frm) {
        ...
        memberUser.categoryBusyIndicator.isActive = true;
        ajax.call(mainPathApi+'MemberGroup/edit', memberUser.selectedItem, 'PUT').success(function (response) {
            memberUser.addRequested = true;
    ...
    memberUser.deleteCategory = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                memberUser.categoryBusyIndicator.isActive = true;
                buttonIsPressed = true;
                ajax.call(mainPathApi+'MemberGroup/getviewmodel', node.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    memberUser.selectedItemForDelete = response.Item;
                    ajax.call(mainPathApi+'MemberGroup/delete', memberUser.selectedItemForDelete, 'DELETE').success(function (res) {
                        if (res.IsSuccess) {
                            memberUser.gridOptions.advancedSearchData.engine.Filters = null;
                            memberUser.gridOptions.advancedSearchData.engine.Filters = [];
                            memberUser.gridOptions.fillData();
    ...
    memberUser.selectContent = function (node) {
        ...
        {       
            ...
            memberUser.categoryBusyIndicator.isActive = true;
            ajax.call(mainPathApi+"membergroup/getAllmember", "?id="+nodeId, 'GET').success(function (response) {
                memberUser.categoryBusyIndicator.isActive = false;
                memberUser.ListItems = response.ListItems;
    ...
    memberUser.openAddModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'memberuser/getviewmodel', "0", 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            memberUser.selectedItem = response.Item;
            if (node.Id != 0 || node.Id) {
...
//#help بررسی عضو و JoinId
    memberUser.CheckMemberToSite= function (frm) { 
        if (frm.$invalid)
            return;
        memberUser.addRequested=true;
        ajax.call(mainPathApi+"memberuser/GetOneByJoinId", {LinkMemberUserId: memberUser.selectedItemMemberInSiteLinkMemberUserId,JoinId: memberUser.selectedItemMemberInSiteJoinId}, "POST").success(function (response) {
            if(response.IsSuccess)
            {
...
//#help اتصال عضو به سایت
    memberUser.addNewMemberToSite= function () { 
        memberUser.addRequestedAddUserInSite=true;
        ajax.call(mainPathApi+"memberuserSite/AddeByJoinId",  {LinkMemberUserId: memberUser.selectedItemMemberInSiteLinkMemberUserId,JoinId: memberUser.selectedItemMemberInSiteJoinId}, "POST").success(function (response) {
            if(response.IsSuccess)
            {
                memberUser.selectedItemMemberInSiteJoinId="";
                memberUser.selectedItemMemberInSiteLinkMemberUserId="";
    ...
    memberUser.openEditModel = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'memberuser/getviewmodel', memberUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response1) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response1);
    ...
    memberUser.addNewContent = function (frm) {
        ...
        memberUser.categoryBusyIndicator.isActive = true;
        memberUser.addRequested = true;
        ajax.call(mainPathApi+'memberuser/add', memberUser.selectedItem, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            memberUser.categoryBusyIndicator.isActive = false;
            if (response.IsSuccess) {
                memberUser.ListItems.unshift(response.Item);
                ...
                memberUser.closeModal();
                //Save inputTags
                //ajax.call(mainPathApi+'memberusergroup/add', memberUser.selectedItem.ContentTags, 'POST').success(function (response) {
    ...
    memberUser.editContent = function (frm) {
        ...
        memberUser.addRequested = true;
        memberUser.categoryBusyIndicator.isActive = true;
        ajax.call(mainPathApi+'memberuser/edit', memberUser.selectedItem, 'PUT').success(function (response) {
            memberUser.addRequested = false;
            memberUser.categoryBusyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    memberUser.deleteContent = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                buttonIsPressed = true;
                ajax.call(mainPathApi+'memberUser/getviewmodel', memberUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    buttonIsPressed = false;
                    rashaErManage.checkAction(response);
                    memberUser.selectedItemForDelete = response.Item;
                    ajax.call(mainPathApi+'memberUser/delete', memberUser.selectedItemForDelete, 'DELETE').success(function (res) {
                        //memberUser.busyIndicator.isActive = false;
                        rashaErManage.checkAction(res);
                        if (res.IsSuccess) {
                            memberUser.replaceItem(memberUser.selectedItemForDelete.Id);
    ...
    memberUser.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", memberUser.fileIdToDelete, 'GET').success(function (response1) {
    ...
    memberUser.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", memberUser.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
   ...
   //Export Report 
    memberUser.exportFile = function () {
        ...
        ajax.call(mainPathApi+'MemberUser/exportfile', memberUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            memberUser.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    memberUser.getCount = function () {
        ajax.call(mainPathApi+"MemberUser/count", memberUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            memberUser.addRequested = false;
            rashaErManage.checkAction(response);
