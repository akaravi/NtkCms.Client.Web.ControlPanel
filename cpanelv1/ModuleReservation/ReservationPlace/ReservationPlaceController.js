app.controller("reservationplaceController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$stateParams', '$filter', '$rootScope', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $stateParams, $filter, $rootScope) {
    ...
    place.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'Reservationplace/getviewmodel', place.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            ...
            if (response.Item.LinkBackgroundImageId != null) {
                ajax.call(mainPathApi+'CmsFileContent/getviewmodel', response.Item.LinkBackgroundImageId, 'GET').success(function (response2) {
    ...
    place.DesignPlace = function (item) {
        ...
        place.myStyles = {
            'background-image': 'url("' + $rootScope.infoDomainAddress + 'mvc/CmsFileContent/Thumbnail/' + item.LinkBackgroundImageId + '")',
    ...
    place.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", place.fileIdToDelete, 'GET').success(function (response1) {
    ...
    place.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", place.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
