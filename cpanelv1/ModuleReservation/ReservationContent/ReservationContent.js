app.controller("reservationContentController", [
    ...
    function (
        ...
        reservationContent.addNewCategoryModel = function () {
            ...
            //reservationContent.modalTitle = ($filter('translate')('Add_Category'));
            reservationContent.addRequested = false;
        ...
        reservationContent.EditCategoryModel = function () {
            ...
            reservationContent.addRequested = false;
            //reservationContent.modalTitle = ($filter('translate')('Edit_Category'));
        ...
        reservationContent.selectContent = function (node) {
            ...
            if (node != null && node != undefined) {
                ...
                reservationContent.attachedFiles = null;
                reservationContent.attachedFiles = [];
                var s = {
                    PropertyName: "LinkCategoryId",
                    IntValue1: node.Id,
        ...
        reservationContent.addNewContentModel = function () {
            ...
            var node = reservationContent.treeConfig.currentNode;
            if (node.Id == 0 || !node.Id) {
                rashaErManage.showMessage($filter('translate')('To_Add_A_reservation_Please_Select_The_Category'));
                buttonIsPressed = false;
                return;
            }
            ...
            reservationContent.addRequested = false;
            //reservationContent.modalTitle = ($filter('translate')('Add_Content'));
            buttonIsPressed = true;
            ajax
        ...
        reservationContent.openEditModel = function () {
            ...
            reservationContent.addRequested = false;
            //reservationContent.modalTitle = ($filter('translate')('Edit_Content'));
            if (!reservationContent.gridOptions.selectedRow.item) {
                rashaErManage.showMessage($filter('translate')('Please_Select_A_Row_To_Edit'));
                return;
            }
            if (reservationContent.gridOptions.selectedRow.item.LinkSiteId != $rootScope.tokenInfo.Item.virtual_CmsSite.Id && !$rootScope.tokenInfo.UserAccessAdminAllowToAllData) {
                rashaErManage.showMessage($filter('translate')('This_reservation_Is_Shared'));
                return;
            }
            buttonIsPressed = true;
            ajax
        ...
        reservationContent.replaceFile = function (name) {
            ...
            // Delete the file
            ajax
                .call(mainPathApi + "CmsFileContent/getviewmodel", reservationContent.fileIdToDelete, "GET")
                .success(function (response1) {
        ...
        reservationContent.saveNewFile = function () {
            ajax
                .call(mainPathApi + "CmsFileContent/add", reservationContent.FileItem, "POST")
                .success(function (response) {
                    if (response.IsSuccess) {
            ...
            if ($("#save-icon" + index).hasClass("fa-save")) {
                if (reservationContent.fileIsExist(uploadFile.name)) { // File already exists
                    ...
                    ajax.call(mainPathApi+"CmsFileContent/getviewmodel", "0", 'GET').success(function (response) {
                        ...
                        // ------- reservationContent.saveNewFile()  ----------------------
                        var result = 0;
                        ajax.call(mainPathApi+"CmsFileContent/add", reservationContent.FileItem, 'POST').success(function (response) {
                            if (response.IsSuccess) {
                                reservationContent.FileItem = response.Item;
                                reservationContent.showSuccessIcon();
                                $("#save-icon" + index).removeClass("fa-save");
