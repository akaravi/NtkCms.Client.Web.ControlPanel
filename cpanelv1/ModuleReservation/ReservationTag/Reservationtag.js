...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 reservationTag.LinkCategoryTagIdSelector = {
    ...
    reservationTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", reservationTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    reservationTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", reservationTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    reservationTag.onSelection = function (node, selected) {
        ...
        reservationTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            reservationTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
