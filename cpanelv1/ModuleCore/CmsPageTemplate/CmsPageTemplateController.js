app.controller("cmsPageTemplateGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsPageTemplategrd.init = function () {
        ajax.call(mainPathApi+"cmsPageTemplate/getall", cmsPageTemplategrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmsPageTemplategrd.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsPageTemplate/getviewmodel', '0', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
	...
	if (frm.$invalid)
        ...
        ajax.call(mainPathApi+'cmsPageTemplate/add', cmsPageTemplategrd.selectedItem, 'POST').success(function (response) {
            cmsPageTemplategrd.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsPageTemplategrd.openEditModal=function() {
        ...
        ajax.call(mainPathApi+'cmsPageTemplate/getviewmodel',  cmsPageTemplategrd.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsPageTemplategrd.selectedItem = response.Item;
            $modal.open({
	...
	if (frm.$invalid)
            return;
        ajax.call(mainPathApi+'cmsPageTemplate/edit', cmsPageTemplategrd.selectedItem , 'PUT').success(function (response) {
            cmsPageTemplategrd.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsPageTemplategrd.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(mainPathApi+'cmsPageTemplate/getviewmodel', cmsPageTemplategrd.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsPageTemplate/delete',  cmsPageTemplategrd.selectedItemForDelete , 'DELETE').success(function (res) {
