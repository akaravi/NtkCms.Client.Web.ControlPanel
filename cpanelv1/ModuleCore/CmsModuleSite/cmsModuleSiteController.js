app.controller("cmsModuleSiteGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsModuleSitegrd.init = function () {
        ...
        // Populate CmsModules
        ajax.call(mainPathApi+"cmsModule/getall", cmsModuleSitegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response1) {
        ...
        // Populate CmsSites
        ajax.call(mainPathApi+"cmsSite/getallwithalias", cmsModuleSitegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response2) {
        ...
        ajax.call(mainPathApi+"cmsModuleSite/getall", cmsModuleSitegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response3) {
    ...
    cmsModuleSitegrd.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsModuleSite/add', cmsModuleSitegrd.selectedItem, 'POST').success(function (response) {
            cmsModuleSitegrd.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsModuleSitegrd.openEditModal = function () {
        ...
        cmsModuleSitegrd.addRequested = true;
        ajax.call(mainPathApi+'cmsModuleSite/getviewmodel', cmsModuleSitegrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            cmsModuleSitegrd.addRequested = false;
            rashaErManage.checkAction(response);
            cmsModuleSitegrd.selectedItem = response.Item;
    ...
    cmsModuleSitegrd.editRow = function (frm) {
        ...
        cmsModuleSitegrd.addRequested = true;
        ajax.call(mainPathApi+'cmsModuleSite/edit', cmsModuleSitegrd.selectedItem, 'PUT').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsModuleSitegrd.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsModuleSite/getviewmodel', cmsModuleSitegrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsModuleSite/delete', cmsModuleSitegrd.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsModuleSitegrd.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmsModuleSite/exportfile', cmsModuleSitegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    cmsModuleSitegrd.getCount = function () {
        ajax.call(mainPathApi+"cmspage/count", cmsModuleSitegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
