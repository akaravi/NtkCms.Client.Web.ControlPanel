app.controller("cmsModuleProcessGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $filter) {
    ...
    cmsModulePrc.init = function () {
        ...
        ajax.call(mainPathApi+"CmsModuleProcess/getall", cmsModulePrc.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmsModulePrc.autoAdd = function () {
        ...
        cmsModulePrc.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsModuleProcess/AutoAdd', '', 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            cmsModulePrc.addRequested = false;
            cmsModulePrc.init();
        }).error(function (data, errCode, c, d) {
    ...
    cmsModulePrc.addNewRow = function (frm) {
        if (frm.$invalid)
            return;
        cmsModulePrc.addRequested = true;
        ajax.call(mainPathApi+'CmsModuleProcess/add', cmsModulePrc.selectedItem, 'POST').success(function (response) {
            cmsModulePrc.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsModulePrc.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'CmsModuleProcess/getviewmodel', cmsModulePrc.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsModulePrc.selectedItem = response.Item;
            $modal.open({
    ...
    cmsModulePrc.editRow = function (frm) {
        ...
        cmsModulePrc.addRequested = true;
        ajax.call(mainPathApi+'CmsModuleProcess/edit' + myControlerAdd, cmsModulePrc.selectedItem, 'PUT').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsModulePrc.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(mainPathApi+'CmsModuleProcess/getviewmodel', cmsModulePrc.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'CmsModuleProcess/delete', cmsModulePrc.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsModulePrc.getCount = function () {
        ajax.call(mainPathApi+"cmsmoduleprocess/count", cmsModulePrc.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
