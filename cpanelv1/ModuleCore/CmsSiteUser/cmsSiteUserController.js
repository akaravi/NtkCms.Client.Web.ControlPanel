app.controller("cmsSiteUserGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    cmsSiteUser.init = function () {
        ...
        ajax.call(mainPathApi+"cmssiteuser/getall", cmsSiteUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            model.SortType = 1;
            model.SortColumn = "Id";
            //ajax.call(mainPathApi+"cmsSite/getallwithalias", model, 'POST').success(function (response) {
            //    rashaErManage.checkAction(response);
            //    cmsSiteUser.cmsSitesListItems = response.ListItems;
            //    ajax.call(mainPathApi+"cmsUser/getall", {}, 'POST').success(function (response) {
            //        rashaErManage.checkAction(response);
        ...
        ajax.call(mainPathApi+"cmssite/getall", cmsSiteUser.gridOptionsSite.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteUser.busyIndicator.isActive = false;
    ...
    cmsSiteUser.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsSiteUser/getviewmodel', '0', 'GET').success(function (response) {
    ...
    cmsSiteUser.addNewRow = function (frm) {
        ...
        cmsSiteUser.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'cmsSiteUser/add', cmsSiteUser.selectedItem, 'POST').success(function (response1) {
            rashaErManage.checkAction(response1);
            if (response1.IsSuccess) {
                ajax.call(mainPathApi+'cmssite/getviewmodel', response1.Item.LinkSiteId, 'GET').success(function (response2) {
                    response1.Item.virtual_CmsSite = { Title: response2.Item.Title };
                    ajax.call(mainPathApi+'cmsuser/getviewmodel', response1.Item.LinkUserId, 'GET').success(function (response3) {
    ...
    cmsSiteUser.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'cmsSiteUser/getviewmodel', cmsSiteUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            cmsSiteUser.selectedItem = response.Item;
            $modal.open({
    ...
    cmsSiteUser.editRow = function (frm) {
        ...
        cmsSiteUser.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'cmsSiteUser/edit', cmsSiteUser.selectedItem, 'PUT').success(function (response1) {
            cmsSiteUser.addRequested = true;
            rashaErManage.checkAction(response1);
            if (response1.IsSuccess) {
                ajax.call(mainPathApi+'cmssite/getviewmodel', response1.Item.LinkSiteId, 'GET').success(function (response2) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsSiteUser/getviewmodel', cmsSiteUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsSiteUser/delete', cmsSiteUser.selectedItemForDelete, 'DELETE').success(function (res) {
...
cmsSiteUser.gridOptionsSite = {
 ...
 //Show Content with Category Id
    cmsSiteUser.selectContent = function (node) {
        ...
        ajax.call(mainPathApi+"cmssiteuser/getall", filterModel, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteUser.ListItems = response.ListItems;
            cmsSiteUser.gridOptions.fillData(cmsSiteUser.ListItems, response.resultAccess);
            cmsSiteUser.gridOptions.currentPageNumber = response.CurrentPageNumber;
    ...
    cmsSiteUser.inputSiteChanged = function (input) {
        ...
        ajax.call(mainPathApi+"cmssite/search", engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteUser.cmsSitesListItems = response.ListItems;
        }).error(function (data, errCode, c, d) {
            rashaErManage.checkAction(data, errCode);
    ...
    cmsSiteUser.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmssite/exportfile', cmsSiteUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsSiteUser.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsSiteUser.getCount = function () {
        ajax.call(mainPathApi+"cmssite/count", cmsSiteUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsSiteUser.addRequested = false;
            rashaErManage.checkAction(response);
