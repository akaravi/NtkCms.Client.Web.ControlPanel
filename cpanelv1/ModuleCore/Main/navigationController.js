app.controller("navigationCtrl", ["$scope", "ajax", "rashaErManage", "$state", "$element", "$translate", "$rootScope", "$filter", function ($scope, ajax, rashaErManage, $state, $element, $translate, $rootScope, $filter) {
    ...
    navCtrl.init = function () {
        ajax.call(mainPathApi+'cmsSite/getAllwithalias', navCtrl.engine, 'POST').success(function (response1) {
        ...
        //MenuRight
        ajax.call(mainPathApi+'CmsCpMainMenu/GetAllMenu', navCtrl.engine, 'POST').success(function (response) {
    ...
    navCtrl.selectCurrentSite = function (item) {
        ...
        ajax.call(mainPathApi+"cmsUser/SelectCurrentSite/", { id: currentSite }, "POST").success(function (response) {
