        ...
        function ($scope, $state, ajax, rashaErManage, $translate, $modal, $rootScope, $modalStack, $filter) {
            ...
            topNavBar.init = function () {
                ...
                if ($rootScope.tokenInfo == undefined || $rootScope.tokenInfo==null || $rootScope.tokenInfo.UserTicketToken==undefined) {
                    ...
                    ajax.call(mainPathApi+"cmsUser/SelectCurrentSite/", {}, "POST").success(function (response) {
            ...
            topNavBar.appLoginById = function (NewSiteid, NewUserid) {
                ...
                if ((NewSiteid != undefined && NewSiteid > 0) || (NewUserid != undefined && NewUserid > 0)) {
                    ...
                    topNavBar.busyIndicator.isActive = true;
                    ajax.call(mainPathApi+"cmsUser/SelectCurrentSite/",
                        {
                            NewUserid: NewUserid,
                            id: NewSiteid,
            ...
            topNavBar.inputSiteChanged = function (input) {
                ajax.call(mainPathApi+"cmssite/search_new", { key: input}, 'POST').success(function (response) {
                    rashaErManage.checkAction(response);
            ...
            topNavBar.inputSiteChangedOld = function (input) {
                ...
                if (input == null)
                    ...
                    engine.Filters.push(
                        { PropertyName: "SubDomain", SearchType: 5, StringValue1: input, ClauseType: 1 });
                } else {
                    engine.Filters.push({
                        PropertyName: "Id",
                        SearchType: 10,
            ...
            topNavBar.setDiskSpaceInfo = function () {
                ajax.call(mainPathApi+"cmsfileconfiguration/all", {}, "POST").success(function (response) {
                    try {
                        rashaErManage.checkAction(response);
                        $rootScope.totalSpace = response.SiteAccess.AllCateSizeUploadMB;
                        $rootScope.usedSpace = Math.floor(response.SiteStorage.SumSizeUploadMB);
  ...
  123: "F12"
            ...
            topNavBar.guideShow = function (area) {
                ...
                if (engine.Filters.length > 0) {
                    //#help# یافت شد
                    topNavBar.busyIndicator.isActive = true;
                    ajax.call(mainPathApi+"cmsguide/getall", engine, 'POST').success(function (response) {
                        rashaErManage.checkAction(response);
                        var introSteps = [];
                        ...
                        topNavBar.busyIndicator.isActive = false;
                        var intro = introJs();
                        intro.setOptions({ 'skipLabel': 'بازگشت', 'nextLabel': 'بعدی', 'prevLabel': 'قبلی' });
