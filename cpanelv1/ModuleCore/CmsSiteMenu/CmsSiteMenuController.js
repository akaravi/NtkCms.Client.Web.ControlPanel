...
app.controller("cmsSiteMenuCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsSiteMenu.init = function () {
        ...
        ajax.call(mainPathApi+"cmsSiteMenu/getAreaType", {}, 'POST').success(function (response) {
            cmsSiteMenu.AreaType = response;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
        ajax.call(mainPathApi+"cmsSiteMenu/getall", cmsSiteMenu.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
        ...
        ajax.call(mainPathApi+'CmsModule/getallmodulename', {}, 'POST').success(function (response) {
        ...
        cmsSiteMenu.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsModulePageDependency/getAllwithAlias', {}, 'POST').success(function (response) {
    ...
    cmsSiteMenu.openAddModal = function () {
        ...
        cmsSiteMenu.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'cmsSiteMenu/getviewmodel', "0", 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            //Load cmspages
            ajax.call(mainPathApi+'cmspage/getallwithalias', { RowPerPage: 200 }, 'POST').success(function (response) {
    ...
    cmsSiteMenu.addNewContent = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsSiteMenu/add', cmsSiteMenu.selectedItem, 'POST').success(function (response) {
            cmsSiteMenu.addRequested = false;
            cmsSiteMenu.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    cmsSiteMenu.openEditModal = function () {
        ...
        cmsSiteMenu.addRequested = true;
        cmsSiteMenu.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'cmsSiteMenu/getviewmodel', cmsSiteMenu.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteMenu.selectedItem = response.Item;
            try {
    ...
    cmsSiteMenu.editContent = function (frm) {
        ...
        cmsSiteMenu.busyIndicator.isActive = true;
        cmsSiteMenu.selectedItem.JsonValues = $.trim(angular.toJson(cmsSiteMenu.treeMenuConfig.treeMenuContent));
        ajax.call(mainPathApi+'cmsSiteMenu/edit', cmsSiteMenu.selectedItem, 'PUT').success(function (response) {
            cmsSiteMenu.addRequested = false;
            rashaErManage.checkAction(response);
            cmsSiteMenu.busyIndicator.isActive = false;
            if (response.IsSuccess) {
    ...
    cmsSiteMenu.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), "آیا می خواهید این منو را حذف کنید؟", function (isConfirmed) {
            if (isConfirmed) {
                cmsSiteMenu.busyIndicator.isActive = true;
                ajax.call(mainPathApi+'cmsSiteMenu/getviewmodel', cmsSiteMenu.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    //rashaErManage.checkAction(response);
                    cmsSiteMenu.selectedItemForDelete = response.Item;
                    ajax.call(mainPathApi+'cmsSiteMenu/delete', cmsSiteMenu.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsSiteMenu.onPageChange = function (selectedPage) {
        ...
        ajax.call(mainPathApi+'CmsModulePageDependency/getviewmodel', selectedPage.LinkPageDependencyId, 'GET').success(function (response1) {
