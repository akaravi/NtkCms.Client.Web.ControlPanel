app.controller("cmsLogController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    cmsLog.init = function () {
        ...
        ajax.call(mainPathApi+"cmsLog/getall", engine, 'POST').success(function (response) {
...
cmsLog.deleteAllRow = function () {
    var filterModelparam = { Filters: [] };
      ...
      ajax
        .call(mainPathApi + "cmsLog/getall", filterModelparam, "POST")
...
cmsLog.listforDel=listforDel;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                console.log(cmsLog.gridOptions.selectedRow.item);
                    ajax.call(mainPathApi+'cmsLog/DeleteBatchList',cmsLog.listforDel , 'DELETE').success(function (res) {
    ...
    cmsLog.deleteRow = function () {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                console.log(cmsLog.gridOptions.selectedRow.item);
                ajax.call(mainPathApi+'cmsLog/getviewmodel', cmsLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    rashaErManage.checkAction(response);
                    cmsLog.selectedItemForDelete = response.Item;
                    console.log(cmsLog.selectedItemForDelete);
                    ajax.call(mainPathApi+'cmsLog/delete', cmsLog.selectedItemForDelete, 'DELETE').success(function (res) {
                        rashaErManage.checkAction(res);
                        if (res.IsSuccess) {
                            cmsLog.replaceItem(cmsLog.selectedItemForDelete.Id);
                            cmsLog.gridOptions.fillData(cmsLog.ListItems);
    ...
    cmsLog.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmsLog/exportfile', cmsLog.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmsLog.getCount = function () {
        ajax.call(mainPathApi+"cmsLog/count", cmsLog.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsLog.addRequested = false;
            rashaErManage.checkAction(response);
