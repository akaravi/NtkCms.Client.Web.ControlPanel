app.controller("cmsUserBadLoginController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmdUserBadLogin.init = function () {
        ...
        ajax.call(mainPathApi+"cmsUserBadLogin/getall", cmdUserBadLogin.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmdUserBadLogin.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsUserBadLogin/getviewmodel', '0', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
	...
	if (frm.$invalid)
        ...
        ajax.call(mainPathApi+'cmsUserBadLogin/add', cmdUserBadLogin.selectedItem , 'POST').success(function (response) {
            cmdUserBadLogin.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmdUserBadLogin.openEditModal=function() {
        ...
        ajax.call(mainPathApi+'cmsUserBadLogin/getviewmodel', cmdUserBadLogin.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmdUserBadLogin.selectedItem = response.Item;
            $modal.open({
	...
	if (frm.$invalid)
            return;
        ajax.call(mainPathApi+'cmsUserBadLogin/edit',  cmdUserBadLogin.selectedItem , 'PUT').success(function (response) {
            cmdUserBadLogin.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmdUserBadLogin.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsUserBadLogin/getviewmodel', cmdUserBadLogin.gridOptions.selectedRow.item.Id , 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsUserBadLogin/delete', cmdUserBadLogin.selectedItemForDelete , 'DELETE').success(function (res) {
...
cmdUserBadLogin.gridOptions = {
   ...
   cmdUserBadLogin.exportFile = function () {
       ...
       ajax.call(mainPathApi+'CmsUserBadLogin/exportfile', cmdUserBadLogin.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
           cmdUserBadLogin.addRequested = false;
           rashaErManage.checkAction(response);
           if (response.IsSuccess) {
   ...
   cmdUserBadLogin.getCount = function () {
       ajax.call(mainPathApi+"CmsUserBadLogin/count", cmdUserBadLogin.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
           cmdUserBadLogin.addRequested = false;
           rashaErManage.checkAction(response);
