app.controller("cmsUserTicketLogController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsUserTicketLog.init = function () {
        ...
        ajax.call(mainPathApi+"cmsUserTicketLog/getall", engine, 'POST').success(function (response) {
    ...
    cmsUserTicketLog.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsUserTicketLog/getviewmodel', '0', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    cmsUserTicketLog.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsUserTicketLog/add', cmsUserTicketLog.selectedItem, 'POST').success(function (response) {
            cmsUserTicketLog.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsUserTicketLog.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'cmsUserTicketLog/getviewmodel', cmsUserTicketLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsUserTicketLog.selectedItem = response.Item;
            $modal.open({
    ...
    cmsUserTicketLog.editRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsUserTicketLog/edit', cmsUserTicketLog.selectedItem, 'PUT').success(function (response) {
            cmsUserTicketLog.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsUserTicketLog.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsUserTicketLog/getviewmodel', cmsUserTicketLog.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsUserTicketLog/delete', cmsUserTicketLog.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsUserTicketLog.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmsUserTicketLog/exportfile', cmsUserTicketLog.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsUserTicketLog.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    cmsUserTicketLog.getCount = function () {
        ajax.call(mainPathApi+"CmsUserTicketLog/count", cmsUserTicketLog.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsUserTicketLog.addRequested = false;
            rashaErManage.checkAction(response);
