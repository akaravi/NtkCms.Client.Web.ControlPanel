app.controller("cmsLocationController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $window, $filter) {
    ...
    cmsLocation.init = function () {
        ...
        ajax.call(mainPathApi+"CmsLocation/getall", cmsLocation.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            ...
            ajax.call(mainPathApi+"CmsLocation/getalllocationtype", {}, 'POST').success(function (response) {
                cmsLocation.locationTypeListItems = response;
                //Filter LocationType
    ...
    cmsLocation.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'CmsLocation/getviewmodel', '0', 'GET').success(function (response) {
    ...
    cmsLocation.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsLocation/add', cmsLocation.selectedItem, 'POST').success(function (response) {
            cmsLocation.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsLocation.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'CmsLocation/getviewmodel', cmsLocation.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            cmsLocation.selectedItem = response.Item;
    ...
    cmsLocation.editRow = function (frm) {
        ...
        ajax.call(mainPathApi+'CmsLocation/edit', cmsLocation.selectedItem, 'PUT').success(function (response) {
            cmsLocation.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsLocation.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'CmsLocation/getviewmodel', cmsLocation.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'CmsLocation/delete', cmsLocation.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsLocation.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmslocation/exportfile', cmsLocation.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    cmsLocation.getCount = function () {
        ajax.call(mainPathApi+"cmsLocation/count", cmsLocation.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
