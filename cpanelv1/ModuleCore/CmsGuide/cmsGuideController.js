app.controller("cmsGuideController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
   ...
   cmsGuide.filePickerFiles = {
    ...
    cmsGuide.init = function () {
        ...
        ajax.call(mainPathApi+"CmsGuide/GetAll", cmsGuide.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
   ...
   cmsGuide.summernoteOptions = {
    ...
    cmsGuide.openAddContentModal = function () {
        ...
        ajax.call(mainPathApi+'CmsGuide/getall', {}, 'POST').success(function (response) {
        ...
        ajax.call(mainPathApi+'CmsGuide/getviewmodel', '0', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    cmsGuide.addNewRow = function (frm) {
      ...
      cmsGuide.stringfyLinkFileIds();
        ...
        ajax.call(mainPathApi+'CmsGuide/add', cmsGuide.selectedItem, 'POST').success(function (response) {
            cmsGuide.busyIndicator.isActive = false;
            cmsGuide.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsGuide.openEditContentModal = function () {
        ...
        ajax.call(mainPathApi+'CmsGuide/getviewmodel', cmsGuide.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
    ...
    cmsGuide.editRow = function (frm) {
      ...
      cmsGuide.selectedItem.LinkFileIds = "";
      cmsGuide.stringfyLinkFileIds();
        ajax.call(mainPathApi+'CmsGuide/edit', cmsGuide.selectedItem, 'PUT').success(function (response) {
            cmsGuide.addRequested = false;
            cmsGuide.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'CmsGuide/getviewmodel', cmsGuide.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'CmsGuide/delete', cmsGuide.selectedItemForDelete, 'DELETE').success(function (res) {
