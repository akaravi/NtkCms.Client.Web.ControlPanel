app.controller("cmsUserController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', "$rootScope", '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $rootScope, $filter) {
    ...
    cmsUser.init = function () {
        ...
        ajax.call(mainPathApi+"cmsUser/getAllGender", {}, 'POST').success(function (response) {
            cmsUser.Gender = response;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });

        ajax.call(mainPathApi+"cmsUser/getall", cmsUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmsUser.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsUser/getviewmodel', '0', 'GET').success(function (response) {
    ...
    cmsUser.addNewRow = function (frm) {
        ...
        if (!emailRegEx.test(cmsUser.selectedItem.Username)) {   //Validate Username
            rashaErManage.showMessage($filter('translate')('Email_Format_Is_Not_Correct'));
            return;
        }
        ...
        ajax.call(mainPathApi+'cmsUser/add', cmsUser.selectedItem, 'POST').success(function (response) {
            cmsUser.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsUser.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'cmsUser/getviewmodel', cmsUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            cmsUser.selectedItem = response.Item;
    ...
    cmsUser.editRow = function (frm) {
        ...
        cmsUser.selectedItem.LinkFileIds = "";
        cmsUser.stringfyLinkFileIds();
        ajax.call(mainPathApi+'cmsUser/edit', cmsUser.selectedItem, 'PUT').success(function (response) {
            buttonIsPressed = false;
            cmsUser.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsUser/getviewmodel', cmsUser.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsUser/delete', cmsUser.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsUser.logintoCp = function (SelectedUserId) {
        ...
        if (SelectedUserId == undefined || SelectedUserId == 'undefined') {
            rashaErManage.showMessage($filter('translate')('User_Not_Selected'));
            return;
        }
        ...
        if (oldSelectedUserId == SelectedUserId) {
            rashaErManage.showMessage($filter('translate')('You_Are_Currently_Working_On_This_User'));
            return;
        }
        ...
        ajax.call(mainPathApi+"cmsUser/SelectCurrentSite/", { NewUserid: SelectedUserId, UserAccessAdminAllowToAllData: oderShowAllDataStatus, UserAccessAdminAllowToProfessionalData: oderShowProfessionalDataStatus, userLanguage: $rootScope.tokenInfo.UserLanguage }, "POST").success(function (response) {
            rashaErManage.checkAction(response);
...
//#help//
    ...
    cmsUser.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", cmsUser.fileIdToDelete, 'GET').success(function (response1) {
    ...
    cmsUser.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", cmsUser.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
...
//#help//
    ...
    cmsUser.exportFile = function () {
        ...
        ajax.call(mainPathApi+'cmsUser/exportfile', cmsUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsUser.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsUser.getCount = function () {
        ajax.call(mainPathApi+"cmsUser/count", cmsUser.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsUser.addRequested = false;
            rashaErManage.checkAction(response);
