app.controller("cmsSiteCategoryGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsSiteCategorygrd.init = function () {
        ajax.call(mainPathApi+"cmsSiteCategory/getall", cmsSiteCategorygrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmsSiteCategorygrd.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'cmsSiteCategory/getviewmodel', '0', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    cmsSiteCategorygrd.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsSiteCategory/add', cmsSiteCategorygrd.selectedItem, 'POST').success(function (response) {
            cmsSiteCategorygrd.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsSiteCategorygrd.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'cmsSiteCategory/getviewmodel', cmsSiteCategorygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsSiteCategorygrd.selectedItem = response.Item;
            $modal.open({
    ...
    cmsSiteCategorygrd.editRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsSiteCategory/edit', cmsSiteCategorygrd.selectedItem, 'PUT').success(function (response) {
            cmsSiteCategorygrd.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsSiteCategorygrd.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(mainPathApi+'cmsSiteCategory/getviewmodel', cmsSiteCategorygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsSiteCategory/delete', cmsSiteCategorygrd.selectedItemForDelete, 'DELETE').success(function (res) {
