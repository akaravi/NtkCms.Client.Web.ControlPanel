app.controller("cmsModulePageDependencyGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $filter) {
    ...
    cmsModulePageDependencygrd.init = function () {
        ...
        ajax.call(mainPathApi+"CmsModulePageDependency/getall", cmsModulePageDependencygrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    cmsModulePageDependencygrd.openAddModal = function () {
        ...
        ajax.call(mainPathApi+'CmsModulePageDependency/getviewmodel', '0', 'GET').success(function (response) {
    ...
    cmsModulePageDependencygrd.addNewRow = function (frm) {
        ...
        cmsModulePageDependencygrd.busyIndicator.isActive = true;
        cmsModulePageDependencygrd.addRequested = true;
        ajax.call(mainPathApi+'CmsModulePageDependency/add', cmsModulePageDependencygrd.selectedItem, 'POST').success(function (response) {
            cmsModulePageDependencygrd.addRequested = false;
            cmsModulePageDependencygrd.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    cmsModulePageDependencygrd.autoAdd = function () {
        cmsModulePageDependencygrd.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsModulePageDependency/AutoAdd', '', 'POST').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    cmsModulePageDependencygrd.openEditModal = function () {
        ...
        buttonIsPressed = true;
        ajax.call(mainPathApi+'CmsModulePageDependency/getviewmodel', cmsModulePageDependencygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            buttonIsPressed = false;
            rashaErManage.checkAction(response);
            cmsModulePageDependencygrd.selectedItem = response.Item;
            $modal.open({
    ...
    cmsModulePageDependencygrd.editRow = function (frm) {
        ...
        cmsModulePageDependencygrd.addRequested = true;
        cmsModulePageDependencygrd.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsModulePageDependency/edit', cmsModulePageDependencygrd.selectedItem, 'PUT').success(function (response) {
            cmsModulePageDependencygrd.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsModulePageDependencygrd.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'CmsModulePageDependency/getviewmodel', cmsModulePageDependencygrd.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'CmsModulePageDependency/delete', cmsModulePageDependencygrd.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsModulePageDependencygrd.getCount = function () {
        ajax.call(mainPathApi+"cmsmodulepagedependency/count", cmsModulePageDependencygrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
