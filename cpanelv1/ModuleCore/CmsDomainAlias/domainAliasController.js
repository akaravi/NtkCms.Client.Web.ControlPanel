app.controller("domainAliasController", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$builder', '$rootScope', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $builder, $rootScope, $window, $filter) {
    ...
    domainAlias.init = function () {
        ...
        ajax.call(mainPathApi+"CmsSiteDomainAlias/getall", domainAlias.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
    ...
    domainAlias.addNewRow = function (frm) {
        ...
        domainAlias.busyIndicator.isActive = true;
        domainAlias.addRequested = true;
        ajax.call(mainPathApi+'CmsSiteDomainAlias/add', domainAlias.selectedItem, 'POST').success(function (response) {
            domainAlias.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    domainAlias.openEditModal = function () {
        ...
        ajax.call(mainPathApi+'CmsSiteDomainAlias/getviewmodel', domainAlias.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            domainAlias.selectedItem = response.Item;
            $modal.open({
    ...
    domainAlias.editRow = function (frm) {
        if (frm.$invalid)
            return;
        domainAlias.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsSiteDomainAlias/edit', domainAlias.selectedItem, 'PUT').success(function (response) {
            domainAlias.addRequested = true;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                domainAlias.addRequested = false;
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(mainPathApi+'CmsSiteDomainAlias/getviewmodel', domainAlias.gridOptions.selectedRow.item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'CmsSiteDomainAlias/delete', domainAlias.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    domainAlias.exportFile = function () {
        ...
        ajax.call(mainPathApi+'CmsSiteDomainAlias/exportfile', domainAlias.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
    ...
    domainAlias.getCount = function () {
        ajax.call(mainPathApi+"CmsSiteDomainAlias/count", domainAlias.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
