app.controller("cmsPageGridDesignCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$stateParams', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $stateParams, $filter) {
    ...
    cmsPageDesign.init = function () {
        ...
        if (cmsPageDesign.classActioName != null && cmsPageDesign.classActioName.toLowerCase().indexOf("withembeddedchild") >= 0) {
            ...
            ajax.call(mainPathApi+'CmsModulePageDependency/getAll', engine, 'POST').success(function (response) {
        ...
        if (!angular.isDefined(cmsPageDesign.masterPageListItems))
            ajax.call(mainPathApi+'cmsPage/getall', filterMasterPage, 'POST').success(function (response) {
                rashaErManage.checkAction(response);
                if (response != null) {
        ...
        if (!angular.isDefined(cmsPageDesign.cmsPageTemplatesListItems))
            ajax.call(mainPathApi+'CmsPageTemplate/getallAvailable', {}, 'POST').success(function (response) {
                rashaErManage.checkAction(response);
        ...
        if (!angular.isDefined(cmsPageDesign.pageAbilityTypeEnum))
            ajax.call(mainPathApi+'CmsPage/getenumpagetype', {}, 'POST').success(function (response) {
                cmsPageDesign.pageAbilityTypeEnum = response;
            }).error(function (data, errCode, c, d) {
                rashaErManage.checkAction(data, errCode);
            });
    }
    ...
    cmsPageDesign.onloadmydata = function () {
        ...
        ajax.call(mainPathApi+'cmsPage/getallwithalias', cmsPageDesign.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            $.each(cmsPageDesign.pageList, function (index, item) { item.Url = item.MainImageSrc });
    ...
    cmsPageDesign.enableMainPage = function (item) {
        ajax.call(mainPathApi+'cmsPage/getviewmodel', item.Id, 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            ...
            ajax.call(mainPathApi+'cmsPage/edit', cmsPageDesign.selectedItem, 'PUT').success(function (response2) {
    ...
    cmsPageDesign.openAddModal = function () {
        ...
        cmsPageDesign.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'cmsPage/getviewmodel', '0', 'GET').success(function (response) {
            rashaErManage.checkAction(response);
            cmsPageDesign.selectedItem = response.Item;
    ...
    cmsPageDesign.addNewRow = function (frm) {
        ...
        ajax.call(mainPathApi+'cmsPage/add', cmsPageDesign.selectedItem, 'POST').success(function (response) {
            cmsPageDesign.addRequested = false;
            cmsPageDesign.busyIndicator.isActive = false;
            rashaErManage.checkAction(response);
    ...
    cmsPageDesign.saveAndGoTohtmlbuilder = function (frm) {
        ...
        if (frm.$name == "frmCmsPageAdd") {   // Functions was called from add modal
            ...
            ajax.call(mainPathApi+'cmsPage/edit', cmsPageDesign.selectedItem, 'PUT').success(function (response) {
                cmsPageDesign.addRequested = false;
                rashaErManage.checkAction(response);
                if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ...
                ajax.call(mainPathApi+'cmsPage/getviewmodel', item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsPage/delete', cmsPageDesign.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    pageTemplateTitle = function (PageTemplateId) {
        ...
        rashaErManage.showMessage($filter('translate')('Template_specifications_not_found'));
        return;
    }

    cmsPageDesign.saveFromAdmin = function (item) {
        ajax.call(mainPathApi+'CmsPage/SetDefaultAdminValuePage', item.Id, 'GET').success(function (res) {
            rashaErManage.checkAction(res);
            if (res.IsSuccess) {
                rashaErManage.showMessage($filter('translate')('Saving_successfully'));
            }
            cmsPageDesign.busyIndicator.isActive = false;
            cmsPageDesign.addRequested = false;
        }).error(function (data2, errCode2, c2, d2) {
    ...
    cmsPageDesign.openDefaultPagesModal = function (item) {
        ...
        if (cmsPageDesign.cmsSiteCategoryListItems == undefined || cmsPageDesign.cmsSiteCategoryListItems.length == 0) {
            ajax.call(mainPathApi+'cmssitecategory/getall', {}, 'POST').success(function (response1) {
                cmsPageDesign.busyIndicator.isActive = false;
                cmsPageDesign.addRequested = false;
    ...
    cmsPageDesign.onSiteCategoryChange = function (categoryId) {
        ...
        cmsPageDesign.addRequested = true;
        cmsPageDesign.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'CmsPage/GetAllDefaultPagesBySiteCategory', { pageId: cmsPageDesign.selectedItem.Id, LinkSiteCategoryId: categoryId, LinkPageDependencyId: cmsPageDesign.dependencyId }, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsPageDesign.cmsDefaultPageListItems = response.ListItems;
                $.each(cmsPageDesign.cmsDefaultPageListItems, function (index, item) { item.Url = item.MainImageSrc });
            }
            cmsPageDesign.busyIndicator.isActive = false;
            cmsPageDesign.addRequested = false;
        }).error(function (data2, errCode2, c2, d2) {
    ...
    cmsPageDesign.saveDefaultJsonValue = function (frm) {
        if (frm.$invalid || cmsPageDesign.LargeImageSelectPage.PageJsonValue == undefined) {
            rashaErManage.showMessage($filter('translate')('Please_select_a_template'));
            return;
        }
        cmsPageDesign.addRequested = true;
        cmsPageDesign.busyIndicator.isActive = true;
        var model = { id: cmsPageDesign.selectedItem.Id, ver: "1", jsonValue: cmsPageDesign.LargeImageSelectPage.JsonValue };
        ajax.call(mainPathApi+'CmsPage/EditHtml', model, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
                cmsPageDesign.closeModal();
            }
  ...
  //#fastUpload
    ...
    cmsPageDesign.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", cmsPageDesign.fileIdToDelete, 'GET').success(function (response1) {
    ...
    cmsPageDesign.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", cmsPageDesign.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    cmsPageDesign.getCount = function () {
        ajax.call(mainPathApi+"cmspage/count", cmsPageDesign.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
