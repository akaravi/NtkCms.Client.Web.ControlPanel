app.controller("cmsPageGridCtrl", ["$scope", "$http", "ajax", 'rashaErManage', '$modal', '$modalStack', 'SweetAlert', '$state', '$window', '$filter', function ($scope, $http, ajax, rashaErManage, $modal, $modalStack, sweetAlert, $state, $window, $filter) {
    ...
    cmsPagegrd.init = function () {
        ajax.call(mainPathApi+"CmsPage/GetAllEnumAction", {}, 'POST').success(function (response) {
            cmsPagegrd.Action = response;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
        ajax.call(mainPathApi+'CmsPageTemplate/getall', {}, 'POST').success(function (response) {
        ...
        ajax.call(mainPathApi+"CmsModulePageDependency/getall", cmsPagegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response2) {
    ...
    cmsPagegrd.runUtilityTools = function (frm) {
        ...
        ajax.call(mainPathApi+'CmsPage/UtilityTools', cmsPagegrd.selectedItemTools, 'Post').success(function (response) {
            rashaErManage.checkAction(response);
    ...
    cmsPagegrd.addNewRow = function (frm) {
        ...
        cmsPagegrd.busyIndicator.isActive = true;
        ajax.call(mainPathApi+'cmsPage/add', cmsPagegrd.selectedItem, 'POST').success(function (response) {
            cmsPagegrd.addRequested = false;
            rashaErManage.checkAction(response);
    ...
    cmsPagegrd.saveAndGoTohtmlbuilder = function (frm) {
        ...
        if (frm.$name == "frmCmsPageAdd") {   // Functions was called from add modal
            ...
            ajax.call(mainPathApi+'cmsPage/edit', cmsPagegrd.selectedItem, 'PUT').success(function (response) {
                cmsPagegrd.addRequested = false;
                rashaErManage.checkAction(response);
                if (response.IsSuccess) {
        ...
        rashaErManage.showYesNo(($filter('translate')('Warning')), ($filter('translate')('do_you_want_to_delete_this_attribute')), function (isConfirmed) {
            if (isConfirmed) {
                ajax.call(mainPathApi+'cmsPage/getviewmodel', item.Id, 'GET').success(function (response) {
                    ...
                    ajax.call(mainPathApi+'cmsPage/delete', cmsPagegrd.selectedItemForDelete, 'DELETE').success(function (res) {
    ...
    cmsPagegrd.onModuleChange = function (moduleId) {
        ...
        ajax.call(mainPathApi+'CmsModulePageDependency/getall', cmsPagegrd.advancedSearchData.engine, "POST").success(function (response) {
            rashaErManage.checkAction(response);
    ...
    cmsPagegrd.LoadModuleOfDependency = function (pageDependencyId) {
        if (pageDependencyId != null) {
            ...
            ajax.call(mainPathApi+'CmsModulePageDependency/getviewmodel', pageDependencyId, 'GET').success(function (response1) {
    ...
    cmsPagegrd.getCount = function () {
        ajax.call(mainPathApi+"CmsModulePageDependency/count", cmsPagegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            rashaErManage.checkAction(response);
...
//Export Report 
    cmsPagegrd.exportFile = function () {
        ...
        cmsPagegrd.addRequested = true;
        ajax.call(mainPathApi+'CmsModulePageDependency/exportfile', cmsPagegrd.gridOptions.advancedSearchData.engine, 'POST').success(function (response) {
            cmsPagegrd.addRequested = false;
            rashaErManage.checkAction(response);
            if (response.IsSuccess) {
