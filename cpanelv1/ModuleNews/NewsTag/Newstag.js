...
//#help/ سلکتور دسته بندی در ویرایش محتوا
 newsTag.LinkCategoryTagIdSelector = {
    ...
    newsTag.replaceFile = function (name) {
        ...
        // Delete the file
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", newsTag.fileIdToDelete, 'GET').success(function (response1) {
    ...
    newsTag.saveNewFile = function () {
        ajax.call(mainPathApi+"CmsFileContent/add", newsTag.FileItem, 'POST').success(function (response) {
            if (response.IsSuccess) {
    ...
    newsTag.onSelection = function (node, selected) {
        ...
        newsTag.selectedItem.previewImageSrc = "CmsFiles/img/loader.gif";
        ajax.call(mainPathApi+"CmsFileContent/getviewmodel", node.Id, "GET").success(function (response) {
            newsTag.selectedItem.previewImageSrc = "/files/" + response.Item.Id + "/" + response.Item.FileName;
        }).error(function (data, errCode, c, d) {
            console.log(data);
        });
    }
